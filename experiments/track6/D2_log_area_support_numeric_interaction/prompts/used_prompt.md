# D2 log_area x 지지체 숫자형 교차항 실험 프롬프트

- 실험 목적: 같은 면적이라도 지지체에 따라 가격 증가 방식이 달라지는지 확인한다.
- 실험 변수: `log_area`, `nant_support`
- 교차항 처리: `log_area`를 문자열로 바꾸지 않고 숫자형으로 유지한다.
- 교차항 생성: 학습 데이터 지지체 카테고리에 대해 `log_area * 해당 지지체 여부` 숫자형 컬럼을 만든다.
- 비교 기준: `log_area + 지지체` 기준 모델과 `log_area + 지지체 + 숫자형 교차항` 모델을 비교한다.
