# D11 artist_name x artwork_age 숫자형 교차항 실험 프롬프트

- 실험 목적: 작가별 특정 시기 또는 오래된 작품의 가격 효과가 다른지 확인한다.
- 실험 변수: `artist_name_ko`, `artwork_age`
- 교차항 처리: `artwork_age`를 문자열로 바꾸지 않고 숫자형으로 유지한다.
- 교차항 생성: 학습 데이터 작품 수 상위 작가에 대해 `artwork_age * 해당 작가 여부` 숫자형 컬럼을 만든다.
- 비교 기준: `artist_name_ko + artwork_age` 기준 모델과 `artist_name_ko + artwork_age + 숫자형 교차항` 모델을 비교한다.
