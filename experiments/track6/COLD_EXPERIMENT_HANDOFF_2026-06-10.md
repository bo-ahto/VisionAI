...
## 추가 (PP-CMIX1): 브레인스토밍 가설 기각 — v0.5 프런티어 확정

작가 가중 학습(MdAPE/p95 악화)·kNN 3원 blend(사다리와 중복, 게이트 미달) 기각. 수집 없는 Cold 점 예측 개선 완전 종결. 유일 잔여 = 검색 수집 확대.


## 최종 (PP-CSRCH2): 수집 확대 파일럿 — 보류, 트랙 완전 종결

warm 150작가 수집·delta 파생·pseudo-cold 3자 비교 결과 **상수 fallback이 수집 real delta를 전 지표에서 이김** — 수집 ROI 없음(현 공식). 마지막 의사결정 닫힘. 최종 서빙: v0.3+v0.4(상수 활성)+v0.5. 재시도 조건: 실제 cold 트래픽 잔차 확보 시.
