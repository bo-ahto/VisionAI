...
- 재료와 지지체를 따로 쓰는 것보다 조합 변수로 묶었을 때 가격 예측력이 개선되는지 확인한다.
...
- 로그면적 + NANT 재료 + NANT 지지체: `log_area, nant_material_idx, nant_support`
- 로그면적 + 재료지지체 조합: `log_area, nant_material_support_bucket`
- 호수 + 재료지지체 조합: `ln_estimated_ho, nant_material_support_bucket`
- 전체 크기 + 재료지지체 조합: `ln_estimated_ho, log_area, width_cm, height_cm, nant_material_support_bucket`
- 로그면적 + 지지체도구 조합: `log_area, nant_support_nant_tool_bucket`
