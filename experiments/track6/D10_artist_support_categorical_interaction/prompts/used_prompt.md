# D10 artist_name x 지지체 범주형 조합 실험 프롬프트

- 실험 목적: 특정 작가가 특정 지지체에서 가격 프리미엄을 갖는지 확인한다.
- 실험 변수: `artist_name_ko`, `nant_support`
- 조합 처리: `artist_name_ko + nant_support`를 범주형 조합 피처로 만든다.
- 비교 기준: `작가명 + 지지체` 기준 모델과 `작가명 + 지지체 + 조합 피처` 모델을 비교한다.
