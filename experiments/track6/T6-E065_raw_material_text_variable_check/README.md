# T6-E065 원본 재료 문구 변수 영향 확인

- 단계: 기본 피처 정의 - 개별 변수 확인
- 가설: 원본 재료 문구에서 만든 키워드 변수는 난트 재료 분류가 놓친 가격 차이를 설명한다.
- 확인 변수: `collected_material_raw`
...
- Warm 변수 추가 모델 학습 피처: `artist_name_ko + ln_estimated_ho + nant_material_idx + nant_tool + collected_material_raw keyword flags`
- Warm 테스트 피처: 학습 피처와 같은 컬럼 사용
- Cold 기준 모델 학습 피처: `ln_estimated_ho + nant_material_idx + nant_tool`
- Cold 변수 추가 모델 학습 피처: `ln_estimated_ho + nant_material_idx + nant_tool + collected_material_raw keyword flags`
