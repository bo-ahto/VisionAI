# D4 artwork_age x 재료 숫자형 교차항 실험 프롬프트

- 실험 목적: 작품 연한에 따라 재료별 가격 효과가 달라지는지 확인한다.
- 실험 변수: `artwork_age`, `nant_material_idx`, `nant_tool`
- 교차항 처리: `artwork_age`를 문자열로 바꾸지 않고 숫자형으로 유지한다.
- 교차항 생성: 학습 데이터 상위 재료 카테고리에 대해 `artwork_age * 해당 재료 여부` 숫자형 컬럼을 만든다.
- 비교 기준: `artwork_age + 재료` 기준 모델과 `artwork_age + 재료 + 숫자형 교차항` 모델을 비교한다.
