"""v3 Group 1.9: Leave-one-medium-out CV.

특정 medium_category 의 작품 전체를 학습에서 제외하고, 그 매체 작품을 예측.
"새로운 매체 (또는 데이터가 적은 매체)에 대한 일반화 능력" 진단.

방법:
- 평가 매체: top-N (default 7) medium_category by n_works
- 각 M에 대해:
  · 학습 데이터: (전체 28,376) - (medium_category == M 작품)
  · 평가 데이터: medium_category == M 작품
- Production routing (warm=XGB / cold=CB+cell calibration)
- 메트릭 per medium: n_total / n_warm / n_cold / overall MdAPE / warm MdAPE / cold MdAPE / W30
- Aggregate vs baseline (full OOF cold/warm MdAPE)

산출물:
    model_test_results/v3_diagnostics/loo_medium.json
    model_test_results/v3_diagnostics/loo_medium_mdape.png

Usage:
    PYTHONPATH=src python3 scripts/v3_loo_medium.py
"""
...
OUT_JSON = DIAG_DIR / "loo_medium.json"
OUT_PNG = DIAG_DIR / "loo_medium_mdape.png"
RANDOM_SEED = 42

LOO_N = int(os.environ.get("LOO_N", 7))
...
def main() -> None:
    ...
    m_counts = pd.Series(medium).value_counts()
    top_media = m_counts.head(LOO_N).index.tolist()
    logger.info("Top-%d media:", LOO_N)
    for m, n in m_counts.head(LOO_N).items():
        logger.info("  %s: n=%d", m, n)

    overall_start = time.time()
    results: list[dict] = []
    for i, m in enumerate(top_media, 1):
        held_out_mask = medium == m
        ...
        logger.info("=" * 80)
        logger.info("[%d/%d] medium=%s n_test=%d (warm=%d cold=%d) n_train=%d",
                    i, LOO_N, m, n_held, n_warm, n_cold, len(train_idx))
    ...
    summary = {
        "config": {
            ...
            "interpretation": (
                "각 medium_category를 학습에서 제외 후 그 매체 작품 예측. baseline (full OOF) "
                "보다 MdAPE가 크게 악화되면 모델이 매체별 패턴에 과도하게 의존했다는 신호. "
                "Scope: top-N 매체 (~98% 작품 커버) 한정. Warm 비교 caveat: baseline warm 9.7%는 "
                "KFold이고 LOO warm은 '같은 작가라도 해당 매체 작품은 전부 제거된 더 어려운 과제'라 "
                "apples-to-apples 비교 아님."
            ),
            "acceptance_gate": "SOFT (observe-and-report) — 운영 gate 아닌 관찰/리포트용 진단",
        },
        "per_medium": results,
        "aggregate": {
            "n_media": len(results),
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(media, rotation=30, ha="right", color="white", fontsize=10)
    ...
    fig.suptitle(
        f"v3 Group 1.9 Leave-one-medium-out CV (top-{LOO_N} media, production routing)",
    ...
    print("\n" + "=" * 100)
    print(f"v3 Group 1.9 Leave-one-medium-out CV Summary (top-{LOO_N})")
    print("=" * 100)
    print(f"\n{'Medium':<18} {'n':>6} {'warm':>5} {'cold':>5} {'MdAPE':>8} {'cold MdAPE':>11} {'warm MdAPE':>11}")
    print("-" * 100)
    for r in results:
        cm = f"{r['cold']['MdAPE']:>8.2f}%" if r["cold"]["MdAPE"] is not None else f"{'-':>9}"
        wm = f"{r['warm']['MdAPE']:>8.2f}%" if r["warm"]["MdAPE"] is not None else f"{'-':>9}"
        print(f"{r['medium']:<18} {r['n_total']:>6} {r['n_warm']:>5} {r['n_cold']:>5} "
