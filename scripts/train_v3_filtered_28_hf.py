"""PR-GALLERY-TIER: 28-feature (29f_hf − gallery_tier) artifact bundle 학습.
...
- Layer 3.A isolated cycle (29f_hf → 28_hf, drop gallery_tier):
...
- integrated_v3_filtered_tuned_28_hf_*           (default unified)
- integrated_v3_filtered_tuned_b_warm_28_hf_*    (B winner / CB copy + XGB retrain)

Usage:
    PYTHONPATH=src python3 scripts/train_v3_filtered_28_hf.py
"""
...
DEFAULT_28_HF_PREFIX = "integrated_v3_filtered_tuned_28_hf"
B_WARM_28_HF_PREFIX = "integrated_v3_filtered_tuned_b_warm_28_hf"
WARM_MIN_COUNT = 5
...
def _save_default_artifacts(
    ...
    cb_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_catboost.cbm"
    xgb_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_xgboost.json"
    label_maps_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_xgboost_label_maps.json"
    warm_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_warm_artists.json"
    metrics_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_metrics.json"
    best_params_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_best_params.json"
    calib_path = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_source_calibration.json"
    ...
    metrics_path.write_text(
        json.dumps(
            {
                "model": DEFAULT_28_HF_PREFIX,
                "data": data_info,
                "features": len(CB_FEATURES_BASE_28_HF),
                "feature_list": CB_FEATURES_BASE_28_HF,
    ...
    calib_path.write_text(
        json.dumps(
            {
                "model_target": "v3_filtered_tuned_28_hf",
                "version": "28_hf_v0_no_op",
                "cold_factors": {},
                "warm_factors": {},
    ...
    logger.info("Default 28_hf artifacts saved (7 file):")
    for p in [
        cb_path,
        xgb_path,
        label_maps_path,
...
def train_default_28_hf(df: pd.DataFrame) -> dict:
    logger.info("=" * 70)
    logger.info("Default 28_hf training")
    logger.info("=" * 70)
    ...
    X, y, groups = _prepare_28_hf_features(df_train)
    ...
    data_info = f"{len(df_train)} = filtered (excluded {n_excluded}), 28_hf (28 + has_followers)"
...
def _save_b_warm_artifacts(
    ...
    cb_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_catboost.cbm"
    xgb_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_xgboost.json"
    label_maps_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_xgboost_label_maps.json"
    warm_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_warm_artists.json"
    calib_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_source_calibration.json"
    metrics_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_metrics.json"
    best_params_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_best_params.json"
    manifest_dst = OUT_DIR / f"{B_WARM_28_HF_PREFIX}_manifest.json"
    ...
    calib_data["model_target"] = "v3_filtered_tuned_b_warm_28_hf"
    calib_dst.write_text(json.dumps(calib_data, ensure_ascii=False, indent=2))

    metrics_dst.write_text(
        json.dumps(
            {
                "model": B_WARM_28_HF_PREFIX,
                "data": data_info,
                "features": len(CB_FEATURES_BASE_28_HF),
                "feature_list": CB_FEATURES_BASE_28_HF,
                "cat_features": CAT_FEATURES_29,
                "best_params": {"catboost": cb_best, "xgboost": xgb_best},
                "note": "PR-GALLERY-TIER: B winner 28_hf. CB copy from default-28_hf / XGB B-retuned on 28_hf warm.",
            },
            ensure_ascii=False,
            indent=2,
        )
        ...
        json.dumps(
            {
                ...
                "xgboost": xgb_best,
                "provenance": {
                    "catboost": "default-28_hf (bit-identical copy)",
                    "xgboost": "B winner warm-only retuned params on 28_hf warm slice",
                },
            },
            ensure_ascii=False,
            indent=2,
        ...
        json.dumps(
            {
                ...
                "cb_provenance": "bit-identical copy from default-28_hf",
                "xgb_provenance": "B-retuned params on 28_hf warm slice",
                "features_count": len(CB_FEATURES_BASE_28_HF),
    ...
    logger.info("B winner 28_hf artifacts saved (8 file):")
    for p in [
        cb_dst,
        xgb_dst,
        label_maps_dst,
...
def train_b_warm_28_hf(default_result: dict) -> None:
    logger.info("=" * 70)
    logger.info("B winner 28_hf training (CB copy + XGB retrain)")
    logger.info("=" * 70)
    ...
    cb_src = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_catboost.cbm"
    label_maps_src = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_xgboost_label_maps.json"
    warm_src = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_warm_artists.json"
    calib_src = OUT_DIR / f"{DEFAULT_28_HF_PREFIX}_source_calibration.json"
...
def main() -> None:
    ...
    train_b_warm_28_hf(result)

    logger.info("=" * 70)
    logger.info(f"PR-GALLERY-TIER training complete (total {time.time()-t0_total:.1f}s)")
    logger.info("=" * 70)
