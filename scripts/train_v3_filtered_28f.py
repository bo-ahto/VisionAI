"""PR-28F: 28-feature artifact bundle 학습 — default + B winner 동시 산출.

Pre-context:
- PR-29F (직전 commit): dead 2 + source 제거 (32→29) — Layer 1+ PASS_WITHIN_NOISE
- Layer 2 audit: for_sale_ratio PSI=8.95 (saatchi 100% 1.0 constant) + serving 4 위치 1.0 하드코딩
- Layer 2.B isolated cycle (29→28, for_sale_ratio 단독 제거):
  - Δ_cold -0.36pp / Δ_warm xgb +0.10pp / Δ_warm ens +0.14pp vs 29f
  - saatchi cold -0.94pp 측정 가능 개선
  - PASS_WITHIN_NOISE (warm 2σ noise 내)
- Codex R3: PASS — 신규 variant 추가 (legacy 29f 유지)

Artifact 산출 (각 prefix별 5+3 file):
- integrated_v3_filtered_tuned_28f_*           (default unified 28f)
- integrated_v3_filtered_tuned_b_warm_28f_*    (B winner 28f / CB copy + XGB retrain)

설계 (PR-29F 패턴 정합):
- default 28f: CB + XGB 신규 학습 (full data / 기존 best_params 재사용)
- b_warm 28f: CB copy from default 28f (cold path bit-identical) + XGB warm-only 재학습
  (warm_only_retuned_best_params.json 재사용 — params는 32f B winner와 동일)

Usage:
    PYTHONPATH=src python3 scripts/train_v3_filtered_28f.py
"""
...
DEFAULT_28F_PREFIX = "integrated_v3_filtered_tuned_28f"
B_WARM_28F_PREFIX = "integrated_v3_filtered_tuned_b_warm_28f"
...
def _save_default_28f_artifacts(
    ...
    """Default 28f artifact bundle 5 file 저장."""
    cb_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_catboost.cbm"
    xgb_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_xgboost.json"
    label_maps_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_xgboost_label_maps.json"
    warm_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_warm_artists.json"
    metrics_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_metrics.json"
    best_params_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_best_params.json"
    calib_path = OUT_DIR / f"{DEFAULT_28F_PREFIX}_source_calibration.json"
    ...
    metrics_path.write_text(
        json.dumps(
            {
                "model": DEFAULT_28F_PREFIX,
                "data": data_info,
                "features": len(CB_FEATURES_BASE_28),
                "feature_list": CB_FEATURES_BASE_28,
                "cat_features": CAT_FEATURES_29,
                "best_params": best_params,
                "metrics_summary": metrics_summary,
                "note": "PR-28F: 29→28 (for_sale_ratio 제거 / Layer 2.B PASS). best_params 재사용.",
            },
            ensure_ascii=False,
            indent=2,
        )
    ...
    calib_path.write_text(
        json.dumps(
            {
                "model_target": "v3_filtered_tuned_28f",
                "version": "28f_v0_no_op",
                "cold_factors": {},
                "warm_factors": {},
                "note": "PR-28F initial: factor=1.0 default. source feature 제거되어 cell-based calibration 자체가 의미 변경됨 — 후속 재설계 필요.",
            },
            ensure_ascii=False,
            indent=2,
        )
    ...
    logger.info("Default 28f artifacts saved:")
    for p in [
        cb_path,
        xgb_path,
        label_maps_path,
...
def train_default_28f(df: pd.DataFrame) -> dict:
    """Default 28f training. Returns dict with cb/xgb models + metadata."""
    logger.info("=" * 70)
    logger.info("Default 28f training")
    logger.info("=" * 70)
    ...
    data_info = (
        f"{len(df_train)} = filtered (excluded {n_excluded}), 28f variant (dead 2 + source 제거)"
    )
...
def _save_b_warm_28f_artifacts(
    cb_src_path: Path,
    ...
    """B winner 28f artifact bundle 8 file 저장."""
    cb_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_catboost.cbm"
    xgb_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_xgboost.json"
    label_maps_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_xgboost_label_maps.json"
    warm_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_warm_artists.json"
    calib_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_source_calibration.json"
    metrics_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_metrics.json"
    best_params_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_best_params.json"
    manifest_dst = OUT_DIR / f"{B_WARM_28F_PREFIX}_manifest.json"

    # 1) CatBoost: COPY (bit-identical cold path to default-28f)
    shutil.copy2(cb_src_path, cb_dst)
    # 2) XGBoost: NEW (B-retuned params on 28f warm)
    ...
    calib_data["model_target"] = "v3_filtered_tuned_b_warm_28f"
        ...
        json.dumps(
            {
                ...
                "metrics_summary": metrics_summary,
                "note": "PR-28F: B winner 28f. CB copy from default-28f / XGB B-retuned warm fit on 28f.",
            },
            ensure_ascii=False,
            indent=2,
        )
        ...
        json.dumps(
            {
                ...
                "xgboost": xgb_best,
                "provenance": {
                    "catboost": "default-28f (bit-identical copy from integrated_v3_filtered_tuned_28f)",
                    "xgboost": "B winner warm-only retuned params (warm_only_retuned_best_params.json) on 28f warm slice",
                },
            },
            ensure_ascii=False,
            indent=2,
        ...
        json.dumps(
            {
                ...
                "cb_provenance": "bit-identical copy from default-28f",
                "xgb_provenance": "B-retuned params (32f B winner) on 28f warm slice",
                "features_count": len(CB_FEATURES_BASE_28),
    ...
    logger.info("B winner 28f artifacts saved:")
    for p in [
        cb_dst,
        xgb_dst,
        label_maps_dst,
...
def train_b_warm_28f(default_28f_result: dict) -> None:
    """B winner 28f: CB copy from default-28f + XGB B-retuned on 28f warm."""
    logger.info("=" * 70)
    logger.info("B winner 28f training (CB copy + XGB retrain)")
    logger.info("=" * 70)
    ...
    cb_default = default_28f_result["best_params"]["catboost"]
    logger.info("XGB B-warm params: %s", xgb_b_best)

    X = default_28f_result["X"]
    y = default_28f_result["y"]
    groups = default_28f_result["groups"]
    warm_mask_arr = _warm_mask(groups)
    X_warm = X.iloc[warm_mask_arr].reset_index(drop=True)
    y_warm = y[warm_mask_arr]
    ...
    # Save artifacts: cb COPY from default-28f, xgb NEW
    cb_src = OUT_DIR / f"{DEFAULT_28F_PREFIX}_catboost.cbm"
    label_maps_src = OUT_DIR / f"{DEFAULT_28F_PREFIX}_xgboost_label_maps.json"
    warm_src = OUT_DIR / f"{DEFAULT_28F_PREFIX}_warm_artists.json"
    calib_src = OUT_DIR / f"{DEFAULT_28F_PREFIX}_source_calibration.json"

    _save_b_warm_28f_artifacts(
        cb_src_path=cb_src,
        ...
        data_info=default_28f_result["data_info"],
    )
...
def main() -> None:
    ...
    # Stage 1: Default 28f
    result = train_default_28f(df)
    _save_default_28f_artifacts(
    ...
    # Stage 2: B winner 28f (CB copy + XGB retune)
    train_b_warm_28f(result)

    logger.info("=" * 70)
    logger.info(f"PR-28F training complete (total {time.time()-t0_total:.1f}s)")
    logger.info("=" * 70)
