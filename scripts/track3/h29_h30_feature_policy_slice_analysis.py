"""Track 3 H29-H30 — feature policy and slice analysis.

Covers:
- H29: whether Warm and Cold should use different feature sets.
- H30: whether feature gains are concentrated in weak slices.
"""
...
OUT_PATH = REPO / "data" / "track3_h29_h30_feature_policy_slice_results.json"
...
def summarize_slice_delta(base_slices: dict, variant_slices: dict) -> dict:
    out = {}
    for name, base in base_slices.items():
        if name not in variant_slices:
            continue
        var = variant_slices[name]
        out[name] = {
            "n": var["n"],
            "delta_median_ape": float(var["median_ape"] - base["median_ape"]),
            "base_median_ape": base["median_ape"],
            "variant_median_ape": var["median_ape"],
        }
    return out
...
def main() -> None:
    ...
    variants = {
        "V0_warm_policy_ho_enhanced": HO_ENHANCED,
        "V1_cold_policy_3d": HO_ENHANCED + ["is_3d_work", "volume_log", "max_side_log", "min_side_log"],
        "V2_cold_policy_relative": HO_ENHANCED + ["medium_area_percentile", "medium_ho_percentile"],
        "V3_cold_policy_3d_relative": HO_ENHANCED
        + ["is_3d_work", "volume_log", "max_side_log", "min_side_log", "medium_area_percentile", "medium_ho_percentile"],
    }

    result = {
        "experiment_id": "H29_H30_feature_policy_slice_analysis",
        "data": {
    ...
    for name, features in variants.items():
        ...
        result["variants"][name] = {
            ...
            "cold": metric(cold[TARGET].values, cold_pred),
            "warm": metric(warm[TARGET].values, warm_pred),
            "cold_slices": slice_metrics(cold, cold_pred),
            "warm_slices": slice_metrics(warm, warm_pred),
        }

    base = result["variants"]["V0_warm_policy_ho_enhanced"]
    for row in result["variants"].values():
        row["delta_median_ape"] = {
            "cold": float(row["cold"]["median_ape"] - base["cold"]["median_ape"]),
            "warm": float(row["warm"]["median_ape"] - base["warm"]["median_ape"]),
        }
        row["slice_delta_vs_base"] = {
            "cold": summarize_slice_delta(base["cold_slices"], row["cold_slices"]),
            "warm": summarize_slice_delta(base["warm_slices"], row["warm_slices"]),
        }
    ...
    result["judgement"] = {
        "best_cold": best_cold,
        "best_warm": best_warm,
        "separate_warm_cold_policy": best_cold != best_warm,
        "cold_candidate": "V1_cold_policy_3d",
        "warm_candidate": "V0_warm_policy_ho_enhanced",
    }
    OUT_PATH.write_text(json.dumps(result, ensure_ascii=False, indent=2))

    print("H29-H30 feature policy and slice analysis")
    print(f"saved: {OUT_PATH}")
    for name, row in result["variants"].items():
        print(
            f"{name:<30} cold={row['cold']['median_ape']:.4f} "
            f"({row['delta_median_ape']['cold']:+.4f}) "
            f"warm={row['warm']['median_ape']:.4f} "
            f"({row['delta_median_ape']['warm']:+.4f})"
        )
        cold_3d = row["cold_slices"].get("3d", {}).get("median_ape")
        cold_2d = row["cold_slices"].get("2d", {}).get("median_ape")
        print(f"  cold_2d={cold_2d} cold_3d={cold_3d}")
