"""Track 3 H13 — material granularity feature confirm.

Tests whether material flags and rarity buckets improve price prediction.
Because medium_category is already categorical, this is an ablation to check
whether explicit material flags help the selected model families.
"""
...
OUT_PATH = REPO / "data" / "track3_h13_material_granularity_results.json"
...
RARITY_COL = "medium_rarity_bucket"


def add_features(df: pd.DataFrame, artist_counts: dict[str, int], medium_counts: dict[str, int]) -> pd.DataFrame:
    df = df.copy()
    ...
    df["is_oil"] = (medium == "oil").astype(int)
    df["is_acrylic"] = (medium == "acrylic").astype(int)
    df["is_mixed_media"] = (medium == "mixed").astype(int)
    df["is_print_like"] = (medium == "print").astype(int)
    df["is_water_based"] = medium.isin(["watercolor", "ink"]).astype(int)
    df["is_drawing"] = medium.isin(["pencil", "pastel"]).astype(int)
    df["is_pigment"] = (medium == "pigment").astype(int)
    df["is_other_medium"] = (medium == "other").astype(int)

    counts = medium.map(medium_counts).fillna(0)
    df[RARITY_COL] = pd.cut(
        counts,
        bins=[-1, 300, 1500, 6000, 1_000_000],
        labels=["rare", "mid", "common", "very_common"],
    ).astype(str)
    return df
...
def main() -> None:
    ...
    medium_counts = train_raw["medium_category"].value_counts().to_dict()

    train = add_features(train_raw, artist_counts, medium_counts)
    warm = add_features(warm_raw, artist_counts, medium_counts)
    cold = add_features(cold_raw, artist_counts, medium_counts)

    variants = {
        "V0_base": (BASE_FEATURES, BASE_CAT),
        "V1_flags": (BASE_FEATURES + MATERIAL_FLAGS, BASE_CAT),
        "V2_rarity": (BASE_FEATURES + [RARITY_COL], BASE_CAT + [RARITY_COL]),
        "V3_flags_rarity": (BASE_FEATURES + MATERIAL_FLAGS + [RARITY_COL], BASE_CAT + [RARITY_COL]),
    }

    result = {
        "experiment_id": "H13_material_granularity_confirm",
        "data": {
            "train_rows": int(len(train)),
            "test_warm_rows": int(len(warm)),
            "test_cold_rows": int(len(cold)),
            "medium_counts_train": {str(k): int(v) for k, v in medium_counts.items()},
        },
        "variants": {},
    }
    ...
    print("H13 material granularity confirm")
