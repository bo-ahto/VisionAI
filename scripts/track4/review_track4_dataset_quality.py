...
def build_report() -> None:
    ...
    lines.append("- 다만 학습 후보 안에 크기 파싱 재검토가 필요한 극단값이 남아 있음")
    lines.append("- `support_category=unknown` 비율이 높아 지지체 피처는 후속 실험에서 보수적으로 사용해야 함")
    lines.append("- 따라서 모델 학습 전 최종 보완 우선순위는 크기 파싱 재검토임")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 5. 학습 후보 내 크기 재검토 필요 항목")
    lines.append("")
    ...
    lines.append("- 해석")
    lines.append("- 일부는 대형 설치/조각일 수 있음")
    lines.append("- 일부는 `2.0 × 65.1 × 53.0 cm`처럼 두께가 첫 번째 값인데 width/height로 잘못 들어간 것으로 보임")
    lines.append("- 일부는 `2227 × 15.8 cm`처럼 단위 또는 소수점 누락 가능성이 있음")
    lines.append("- 조치")
    lines.append("- 3개 치수에서 가장 작은 값이 두께로 보이는 회화/평면 작품은 depth로 재배치하는 규칙을 검토함")
    lines.append("- width/height 1000cm 초과 값은 원문 URL 또는 이미지 기준으로 cm/mm/소수점 오류 여부를 확인함")
    lines.append("- 크기 재검토 전에는 `aspect_ratio`, `area_cm2`, `log_area`, `estimated_ho` 계열 피처 해석에 주의해야 함")
    lines.append("")
    ...
    lines.append("- 현재 데이터셋은 split 구조 검증은 통과했지만, 크기 파싱 보완 후 다시 생성하는 것이 안전함")
