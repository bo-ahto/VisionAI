#!/usr/bin/env python3
"""PP-CSIM10: Cold gallery-tier feature validation.

Validate whether user-selected gallery information can improve strict Cold.
The experiment keeps the selected k160 q45 structure and adds gallery-tier
features derived from the frozen Track4 gallery audit/dictionary data.
"""
...
EXP_ID = "PP-CSIM10"
SLUG = "PP-CSIM10_cold_gallery_tier_feature_validation"
TITLE = "Cold 갤러리 티어 피처 검증"
EXP = BASE_EXP_DIR / SLUG
OUT = EXP / "outputs"
REPORTS = EXP / "reports"
ARTIFACTS = EXP / "artifacts"
DOC_MD = REPO / "docs" / "track6" / "experiments" / "pp_csim10_cold_gallery_tier_feature_validation_summary.md"
...
def load_gallery_audit_map() -> pd.DataFrame:
    ...
    audit = audit[[
        ...
        "gallery_name_raw": "user_gallery_name_raw",
        "gallery_key": "user_gallery_key",
        "gallery_tier_validated": "user_gallery_tier_validated",
        "gallery_ref_type": "user_gallery_ref_type",
        "gallery_audit_status": "user_gallery_audit_status",
    })
    return audit
...
def add_gallery_features(frames: list[pd.DataFrame]) -> list[pd.DataFrame]:
    ...
    for frame in frames:
        ...
        out["user_gallery_tier_score"] = tier.map(tier_score).astype("float64")
...
def main() -> None:
    ...
    md = "\n".join([
        ...
        "",
        "## 3. 갤러리 티어 커버리지",
        md_table(coverage_df, ["split", "n", "gallery_tier_available_n", "gallery_tier_available_rate", "gallery_audit_ok_n"]),
        "",
        "## 4. 가격대별 진단",
