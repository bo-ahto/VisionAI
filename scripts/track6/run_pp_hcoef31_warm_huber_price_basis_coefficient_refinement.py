#!/usr/bin/env python3
"""Run PP-HCOEF31: p95-neutral directional micro correction.

HCOEF30 proved that validation-consensus segment gating can make OOF/repeated
metrics very strong, but the fixed p95 guard still blocks adoption. HCOEF31
keeps the same validation-only selection principle and adds two extra brakes:
...
2. Micro correction:
   Even when a segment is allowed, the movement is clipped to a very small cap.
...
EXP_ID = "PP-HCOEF31"
EXP_SLUG = "PP-HCOEF31_warm_huber_price_basis_coefficient_refinement"
...
class Policy:
    ...
    def candidate(self) -> str:
        return (
            f"hcoef31_{self.source_tag}_{self.objective}"
...
def generate_predictions(seed: pd.DataFrame, existing: pd.DataFrame, policies: list[Policy], policy_rows: pd.DataFrame, rules: pd.DataFrame) -> pd.DataFrame:
    ...
    for policy in policies:
        ...
        out["method"] = "directional_micro_p95_neutral_gate"
...
def write_report(
    ...
    md = "\n".join(
        [
            f"# {EXP_ID} Warm Huber p95-neutral directional micro correction",
            "",
    ...
    (DOC_ROOT / "pp_hcoef31_warm_huber_price_basis_coefficient_refinement_summary.md").write_text(md, encoding="utf-8")
    (DOC_ROOT / "pp_hcoef31_warm_huber_price_basis_coefficient_refinement_summary.html").write_text(h24.md_to_html(md), encoding="utf-8")
...
def write_config(sources: pd.DataFrame, policies: pd.DataFrame) -> None:
    payload = {
        ...
        "source_experiments": ["PP-HCOEF29", "PP-HCOEF30"],
        "baseline": BASELINE,
        "reference": REFERENCE,
        ...
        "caps": CAPS,
        "top_ns": TOP_NS,
        "selection_rule": "validation row/artist OOF segment consensus plus residual direction consensus; fixed test and 0604 confirmation only",
    }
