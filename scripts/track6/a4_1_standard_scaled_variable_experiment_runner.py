#!/usr/bin/env python3
"""A4-1 Track6 numeric StandardScaler experiment runner.

This runner is intentionally separated from the fixed variable runner because
A4-1 must treat artwork_year and artwork_age as continuous numeric features
with StandardScaler preprocessing. The split, model set, metrics, label usage,
and output format stay fixed for reproducibility.
"""
...
def write_experiment_data_files(
    ...
    feature_cols = ["_track6_row_id", *config["export_feature_columns"]]
    ...
    manifest = {
        ...
        "feature_processing": {
            ...
            "categorical_features": [
                col
                for col in all_required_feature_columns(config)
                if col not in set(config.get("numeric_features", []))
            ],
        },
        "data_policy": {
