...
Runbook: docs/pr_28_hf_stage_3_5_activation_runbook_20260511.md (§2.3)

PR-WARM-B 패턴 정합 + variant naming만 28_hf로 매핑.
Codex R1 권고: non-inferior 기준 ("개선 입증"보다 "regression 부재 확인")

Usage:
    python3 scripts/pr_28_hf_daily_shadow_metric.py --days 7

Output:
    Daily Δ_MdAPE distribution (primary vs shadow) + cold/warm slice + per-source.
    Compares primary (default v3_filtered_tuned, 32f) vs shadow (v3_filtered_tuned_28_hf, 28_hf).
...
# PR-WARM-B variant pattern과 동일. 본 28_hf는 base replacement (32→28).
SHADOW_VARIANT = "v3_filtered_tuned_28_hf"
...
def _fetch_daily_overall(conn, days: int) -> list[dict]:
    ...
    cols = (
        "day", "n_requests",
        "primary_mdape_overall", "shadow_mdape_overall",
        "primary_mdape_cold", "shadow_mdape_cold",
        "primary_mdape_warm", "shadow_mdape_warm",
        "n_warm", "n_cold",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=Path("model_test_results/pr_28_hf_daily_shadow_report.json"),
    )
    ...
    try:
        ...
        for d in daily:
            ...
            logger.info(
                ...
                marker, d["day"], d["n_requests"], d_o, d["n_warm"], d["n_cold"],
            )
        ...
        for d in per_source:
            ...
            logger.info(
                ...
                d["day"], d["source"], d["n_requests"],
                float(p), float(s), float(s) - float(p),
            )
