...
COLUMN_DECISIONS = [
    ...
    ("medium_category", "매체분류", "categorical 정규화", "표준 분류 (oil/acrylic/...)",
     ...
     "medium 텍스트 의 키워드 매칭 → 표준 8 분류 (운영 classify_medium 함수)",
     "MEDIUM_RULES 키워드: oil/acrylic/ink/watercolor/pigment/mixed/pastel/pencil → 매칭 / fallback 'other'"),
    ...
    ("support_type", "지지체분류", "categorical 정규화", "표준 분류 (canvas/paper/...)",
     "보존", "categorical 분포 OK",
     "medium 텍스트 의 키워드 매칭 → 표준 7 분류 (운영 classify_support 함수)",
     "SUPPORT_RULES 키워드: canvas/linen/paper/panel/silk/metal → 매칭 / fallback 'other'"),
    ...
    ("year_made", "제작연도", "수집/enrichment", "Artsy=수집 / Saatchi=PR #51 enrichment",
     ...
     "Artsy: source API 직접 / Saatchi: detail page HTML regex 추출 (PR #51 / 97.90% fill)",
     "Saatchi enrichment: r'<h5>Year Created:</h5>.*?<p>(\\d{4})</p>'"),
    ...
    ("artist_birth_year", "작가생년", "수집/regex추출", "Artsy=수집 / Saatchi=bio regex",
     ...
     "Artsy: source API 의 birthday/birthYear / Saatchi: bio free-text 의 regex 5 patterns 추출 (9.26% fill)",
     "Saatchi: r'born|b\\.\\s+(?:in\\s+)?(?:on\\s+)?...(\\d{4})' 등 5 pattern 의 첫 매칭"),
    ...
    ("career_stage", "경력단계점수", "계산", "multi-factor v2 score (0-8)",
     "보존", "계산",
     "career_stage_v2_score 의 3 component 합계",
     "score = age_score (0-3) + activity_score (0-3) + market_presence (0-2) / age=(age-30)/12, activity=ln(solo+0.7×fair+0.3×group), market=ln_followers/6"),
    ...
    ("solo_count", "개인전수", "수집→정규화", "exhibitions/bio 추정",
     "보존", "수집→정규화",
     "exhibitions / bio 텍스트 의 정규식 매칭 → 'solo' line count",
     "solo_count = count(re.findall('solo|개인전', combined_text))"),
    ("group_count", "단체전수", "수집→정규화", "exhibitions/bio 추정",
     "보존", "수집→정규화",
     "exhibitions / bio 텍스트 의 정규식 매칭 → 'group' line count",
     "group_count = count(re.findall('group|단체전', combined_text))"),
    ("fair_count", "아트페어수", "수집→정규화", "exhibitions/bio 추정",
     "보존", "수집→정규화",
     "exhibitions / bio 텍스트 의 정규식 매칭 → 'fair' line count",
     "fair_count = count(re.findall('fair|페어|art fair', combined_text))"),
    ...
    ("gallery_city_count", "갤러리도시수", "계산", "count(gallery_cities)",
     "보존", "계산",
     "gallery_cities 문자열 의 콤마 분리 count",
     "gallery_city_count = len(gallery_cities.split(','))"),
    ...
    ("mediums_json", "매체배열(JSON)", "categorical 정규화", "복수 medium 의 JSON 배열",
     ...
     "visionai parser 의 한국어 medium 분류 결과 JSON 직렬화",
     "mediums_json = json.dumps([parser.medium_leaves]) / 예: '[\"유채\", \"파스텔\"]'"),
    ("supports_json", "지지체배열(JSON)", "categorical 정규화", "복수 support 의 JSON 배열",
     "분리", "코덱스: UI 표시 용 / cleansed CSV 본체 미포함",
     "visionai parser 의 한국어 support 분류 결과 JSON 직렬화",
     "supports_json = json.dumps([parser.support_leaves]) / 예: '[\"캔버스\"]'"),
]
