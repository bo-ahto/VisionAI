...
COLUMN_DECISIONS = [
    ...
    ("medium_category", "매체분류", "categorical 정규화", "표준 분류 (oil/acrylic/...)",
     ...
     "medium 텍스트 의 substring rule-first 매칭 (운영 classify_medium / prepare_primary_market_dataset.py:93)",
    ...
    ("group_count", "단체전수", "수집/source-conditional", "Artsy=shows_data 직접 / Saatchi=텍스트 추정",
     "보존", "수집 / source-conditional 정규화",
     "Artsy: artist_shows API 의 group show count 직접 / Saatchi: extract_exhibition_counts (prepare_saatchi_dataset.py:117) 의 line-heuristic count",
    ...
    ("fair_count", "아트페어수", "수집/source-conditional", "Artsy=shows_data 직접 / Saatchi=텍스트 추정",
     "보존", "수집 / source-conditional 정규화",
     "Artsy: artist_shows API 의 fair count 직접 / Saatchi: extract_exhibition_counts (prepare_saatchi_dataset.py:117) 의 line-heuristic count",
