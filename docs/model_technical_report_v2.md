...
>
> **📐 명칭 표기 정책**: 콜론30 피드백 Q9(2026-04-24)에 따라 작가 분류 시 "신진/중견/원로" 같은 시장 통념적 라벨은 사용하지 않는다. 모든 분류는 **수치형 점수(`career_stage` 0~8)** 또는 **운영 등급(A/B/C/D)** 으로만 표기한다. 본 문서 본문도 같은 정책을 따른다.
>
...
| **warm / cold (작가 분류)** | — | warm = 학습 시 ≥5건 작품이 있던 작가, cold = 신규 작가 또는 학습 작품 부족. 라우팅 분기 기준. |
| **OOF / GroupKFold / KFold** | Out-Of-Fold, K-Fold cross validation | 5-fold 교차검증의 검증 fold 예측. GroupKFold는 작가 단위로 묶어서 분할(신규 작가 시뮬레이션). |
| **콜론30** | 협력 회사명 | 본 모델의 1차 시장 데이터·갤러리 티어·피드백 등을 제공하는 협력사. v1 보고서까지는 "협력자"로 표기됐으나 v2부터 회사명 명시. |
...
위 표의 "피처 수 37 → 32" 변화를 자세히. 자세한 진단·발견 경위는 §4.4 참조.

| # | 제거된 피처 | 의미 | 학습 데이터 분포 | 서빙 시점 값 | 왜 제거? |
|:-:|---|---|---|---|---|
| 1 | `career_age` | 첫 활동 후 경과 연수 (=2026 - 첫 전시 연도) | Artsy 76% non-null, range 0~16 | **0 고정** — DB에 컬럼 없음, `artist_matcher.py`가 항상 0 반환 | 학습은 분포가 있는 신호로 학습됐는데 서빙은 모두 0 → silent drift. age 정보는 `career_stage_v2_score`의 `age_score` (birth_year 기반)로 흡수. |
| 2 | `work_age` | 작품 제작 후 경과 연수 (=2026 - 작품 연도) | 정상 계산 | **0 고정** — API 요청 스키마에 작품 연도 필드 없음 | 서빙 시 작품 연도를 받지 못함 → 학습 신호 활용 불가. drift. |
| 3 | `vintage_premium` | 빈티지 프리미엄 계수 (오래된 작품에 가산) | 학습 시 계산 | **0 고정** | `work_age`에 의존 → 서빙에서 0 → drift. **XGB importance gain 1.53**으로 가장 큰 문제. |
| 4 | `freshness_discount` | 신선도 할인 계수 (최신 작품에 약간의 할인) | 학습 시 계산 | **0 고정** | `work_age` 의존 → drift. |
| 5 | `gallery_name` | 갤러리명 (categorical, 학습 vocab 59개) | 59개 갤러리명 (예: "Kukje Gallery", "Gallery Hyundai") | **`'Gallery'` 또는 `'Saatchi Art'` 2개로 하드코딩** — `artist_matcher.py`가 vocab의 작은 부분만 사용 | 학습 vocab과 서빙 vocab 불일치 → 매번 unseen sentinel encoding. 신호 활용 0. |

**왜 줄였더니 정확도가 좋아졌나?** 이 5개 피처는 학습 시 의미 있는 신호로 사용됐지만, 서빙 시 모두 같은 값(주로 0)이 들어가면서 모델은 "이 값이 0이면 X" 같은 잘못된 패턴을 적용하고 있었다. 즉 학습한 신호가 production에서 노이즈로 변질된 상태 → 제거하니 noise가 사라지면서 cold MdAPE 40.6 → 39.4로 개선. **train/serve drift는 정확도 그래프엔 안 잡히지만 production에서 silent하게 해를 끼친다.**

---

## 2. 콜론30 피드백 24건 → 모델 변경 매트릭스

콜론30 피드백은 두 차례에 걸쳐 들어왔다.
...
> **반영 상태 범례**: ✅ 반영 완료(모델/문서) · 🔄 부분 반영(코어는 반영, 일부 후속) · 📋 추후 계획(운영/콜론30/정책 협조 후) · ❌ 미반영(검토 후 미채택)

| # | 콜론30 질문 (요약) | 데이터 검증 결과 | 적용된 변경 | 반영 상태 | PR |
|:-:|---|---|---|:-:|:-:|
| Q1 | "profile_completeness 8.41%는 운영 정책에 반영해야" | 사실 — 프로필 충실도가 작가 전문성 프록시 | 모델 변경 없음 (피처는 v1부터 이미 사용 중). **운영 가이드 작성**(작가 등록 가이드 강화)은 별도 의제 | 📋 운영 가이드 추후 | (운영) |
| Q2 | "갤러리 티어 v3 리스트 89개 공유. 모델에 반영해달라" | Artsy 갤러리 66개 중 11개만 매칭 (13.2%) | PR #19에서 매칭 분석 + 가격 분리도 측정 → Top 30 미매칭 명단 산출. **Phase 1B(실 학습 적용)는 콜론30 Top 30 검수 후 진행** | 🔄 분석 완료, 학습 적용은 콜론30 검수 대기 | #19 |
| Q3 | "원작/에디션 0.00% 중요도는 데이터 편향 의심" | 99.9% Unique (Saatchi=100%, Artsy=99.5%, Edition=34건/0.1%). 분산 부족이 원인 | v2 모델은 그대로 (분산 부족이라 학습 불가). **Edition 케이스는 printbakery 별 모델로 분리 검토** 의제 | 📋 정책 결정 + 별 모델 검토 | (정책) |
| Q4 | "한국 갤러리 직접 등록 작가는 어떻게 처리?" | 현재 Cold path(CatBoost) 분기 + `manual_overrides` API로 이미 처리 가능 | 코드는 v1부터 존재 — UX/UI 보강이 필요. **갤러리 입력 폼**(생년·솔로전·페어 등)은 운영팀 협조 의제 | 📋 갤러리 입력 UX 추후 | (운영) |
| Q5 | "외부 플랫폼 등록 작품수는 시장 활동 0.2%만" | 사실 — `artist_total_works`는 온라인 활동성 + 데이터 소스 식별 시그널 혼합 | 모델 자체 변경 없음 (피처 의미 보고서에 caveat 추가). **갤러리 자체 전시·페어 입력 데이터 수집** 시 신호 보강 가능 | 📋 갤러리 입력 데이터 수집 추후 | (운영) |
| Q6 | "플랫폼은 Artsy + Saatchi만?" | v3-filtered-tuned 기준 정확히 두 개 | **본 v2 본문 §3·§5에 명시** | ✅ 문서 정정 완료 | (문서) |
| Q7 | "C/D 가격 범위 너무 넓음 (50%/70%)" | A 등급 m=0.20에서 80% 커버리지 71.5% (좁음). C/D 권장 m≈0.9 (사실상 무용) | PR #22에서 production-time per-grade m 권장치 산출 ([§6.2](#62-등급-마진-production-time-재캘리브레이션)). **권장치 적용은 정책 결정 보류 중** (옵션 1/2/3 §6.2 참조) | 🔄 측정·권장치 산출 완료, 적용은 정책 결정 대기 | #22 |
| Q8 | "개인전/단체전 직접 피처에 있나?" | `solo_count`/`group_count`/`fair_count`는 학습 데이터 컬럼은 있지만 v1 37개 모델 입력에 미포함 | PR #20에서 `career_stage_v2_score` **입력으로 통합** (직접 피처화는 v4 실험에서 과적합 사례 있어 보류). 콜론30 의문 해소를 위해 본 v2 §4.4에 명시 | ✅ 통합 완료 (직접 피처화는 미채택) | #20 |
| Q9 | "career_stage 명칭 변경. 신진/중견/원로 라벨 부적절" | Stage 3=0건으로 사문화. 명명도 시장 합의 없음 | PR #20에서 v2 정의(연속 0~8 점수) + 라벨 수치형으로 통일. **본 v2 본문 전체에서 시장 통념 라벨 사용 금지** | ✅ 모델·문서 모두 반영 완료 | #20 |
| Q10 | "갤러리 유형/서울 가중치/해외 갤러리 합리성" | 단순 휴리스틱 (`has_seoul`, `has_international` 등 합 2.5%) | PR #19 갤러리 티어 분석. **Phase 1B(콜론30 리스트 직접 적용 + Frieze/Art Basel 글로벌 피처)는 콜론30 검수 후 진행** | 🔄 분석 완료, 학습 적용은 콜론30 검수 대기 | #19 |

**반영 상태 집계**: ✅ 완료 3건 (Q6, Q8, Q9) · 🔄 부분 반영 3건 (Q2, Q7, Q10) · 📋 추후 계획 4건 (Q1, Q3, Q4, Q5)

### 2.2 실험계획서 피드백 14건 (P-Q1~P-Q14)

| # | 콜론30 제안 | 처리 | 반영 상태 |
|:-:|---|---|:-:|
| P-Q1 | "Career Age 추가 (첫 개인전 후 경과 연수)" | 초안에 포함 → 학습/서빙 drift 발견(서빙 시 0, DB 스키마 컬럼 없음) → PR #20에서 **제거**. age 정보는 birth_year 기반 `age_score`로 흡수 | 🔄 부분 — age는 흡수, career_age 자체는 미반영 (DB 협조 필요) |
| P-Q2 | "전시 갤러리 티어 가중 합산 (국제갤러리 vs 카페갤러리)" | PR #19 매핑 분석 + 가격 분리도 측정. 매칭 11/66 → Phase 1B 대기 | 🔄 분석 완료, 학습 적용은 콜론30 검수 후 |
| P-Q3 | "1호 작품 점당 가격 (호당가 아님)" | v1 `is_small` 피처(중요도 0.73%)로 부분 반영. 작가별 점당가 룩업은 미반영 | 🔄 부분 — `is_small`만, 작가별 룩업 추후 |
| P-Q4 | "3호 미만 = 10호 가격의 50~60%" | v1 `support_factor` + `ho_x_support` 교차항으로 일부 반영. 명시 룩업 미반영 | 🔄 부분 |
| P-Q5 | "F/P/M/S 호수 변환 면적 차이" | v1: 면적 → F형 호수 매칭(24개 표준 캔버스). aspect_ratio로 타입 추정 가능하나 미반영 | 🔄 F형 매칭만, 타입별 분리 미반영 |
| P-Q6 | "지지체(canvas/paper) + 유니크/에디션 구분 필수" | v1: `support_factor` 5단계 + `is_unique`/`is_edition` 피처. 단 99.9% Unique로 분산 부족 | ✅ 지지체 반영 / 🔄 에디션은 분산 부족 |
| P-Q7 | "서울옥션 크롤러 활용?" | B 모델(경매 낙찰가) 영역. 본 1차 시장 모델은 갤러리 데이터 전용 | ❌ 범위 외 (B 모델 의제) |
| P-Q8 | "갤러리 명성 ↔ 가격(+) 한국 시장 방향" | v1 보고서 §2.3에 한국 시장 맥락 추가. PR #19에서 갤러리 티어 매칭 분석 | ✅ 문서 + 분석 반영 |
| P-Q9 | "전시 갤러리/아트페어 티어" | Q2/Q10과 동일 — Phase 1B에 통합 | 🔄 분석 완료, 적용은 검수 후 |
| P-Q10 | "서울옥션 크롤러" | P-Q7과 동일 | ❌ 범위 외 |
| P-Q11 | "비평(전문가 평가) 정량화" | Phase 2+ 의제 | 📋 추후 (Phase 2+) |
| P-Q12 | "김윤신 1명 작가 편향" | 갤러리 데이터 확장 의제 | 📋 추후 |
| P-Q13 | "작품 연식 × 경력 인터랙션" | v1 `vintage_premium`, `freshness_discount` 피처 시도 → **PR #20에서 train/serve drift로 제거** | ❌ drift로 제거. 재도입 시 작품 연도 입력 필드 + 모델 재학습 필요 |
| P-Q14 | "레지던시 선정 횟수" | 데이터 수집 미완 | 📋 추후 (데이터 수집 의존) |

**반영 상태 집계**: ✅ 완료 1건(P-Q8) + ✅/🔄 혼합 1건(P-Q6) · 🔄 부분 6건 · 📋 추후 3건 · ❌ 미반영/범위외 3건
...
콜론30(2026-04-26)가 갤러리/기관 티어 리스트 v3 (`data/art_gallery_tier_list_v3.xlsx`, 89개)를 공유. v1 모델의 `gallery_tier`는 도시 수·평균가·작품 수 휴리스틱(중요도 0.30%)이었기에 콜론30 리스트로 대체 가능성을 탐색.
...
**시사점**: 1차 시장 가격 모델의 정확도 한계는 **모델 코드보다 데이터**에 있다. 액션 플랜의 큰 폭 개선 후보는 모두 콜론30/운영 데이터 협조 의존(Phase 1B 갤러리 티어 검수, 갤러리 입력 UX, DB schema career_age 추가).
...
3. **갤러리 티어 v3 매칭 81% 미해결** (콜론30 협조): Artsy 갤러리 66개 중 11개만 콜론30 리스트와 매칭. PR #19 Top 30 미매칭 명단(영문명+한글 추정) 검수 필요 — 완료 시 Phase 1B에서 갤러리 등급 신호 회복(예상 -1~2%p A 등급 MdAPE 개선).
