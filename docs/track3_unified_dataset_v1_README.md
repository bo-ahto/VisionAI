...
| · | `is_outlier` | binary (0/1) | 100% | **이상값 flag (학습 시 0만 사용)**. 옵션 B 조건: area<50 OR area>100K cm² OR 단가<100원/cm² OR price<200K원 OR ho=0 (1호 미만 미니어처) OR ho>200 OR ko_len>15자. 학습 시 `df[df.is_outlier==0]` 필터링 권장. |
...
- **v13 (2026-05-11)**: 41,365 rows / **22 cols**. `is_outlier` flag 컬럼 추가 (옵션 B: 학습 안정성 우선). 옵션 B 조건 = area<50 OR area>100K cm² (단위 오류) + 단가<100원/cm² OR price<200K원 (데이터 누락 의심) + ho=0 미니어처 OR ho>200 OR ko_len>15자 (외국 작가). 총 **1,228 rows (3.0%) flag → 학습 셋 40,137 rows (97.0%)**. 학습 시 `df[df.is_outlier==0]` 필터링. Source 별 유지율: artsy 96.8% / saatchi 97.1% / artue 97.6%. 별도 `track3_unified_v1_train.csv` 학습용 export. Outlier 사유 분포: 미니어처(ho=0) 807건 / 단가극저 321건 / 가격극저 96건 / 극대크기 85건 / 극소크기 35건 / 외국작가(>15자) 2건.
- **v12 (2026-05-11)**: 41,365 rows / **21 cols**. `artist_name_ko` 추가 audit (의심 패턴 분류 점검). (A) 짧은 한글 + fallback char, (B) 한자/숫자 metadata 포함, (C) 중간 fallback char (5+자), (D) outlier 작가 — 각 카테고리에서 빈도 Top 추출. 발견: (1) `pyo→표` 음절 누락 (EUN PYO HONG → 홍은표) (2) `suh` PRIORITY 누락 (Suh Hee Sun → 서희선) (3) Yong=용 surname 누락 (Jeremy Yong → 용제레미) (4) 단일 토큰 분기에서 영어 사전 lookup 누락 (Vakki). 추가 ENGLISH dict (~30 entries): Kris, Denis, Molly, Johnny, Melody, Stella, Jeremy, Tina, Lina, Nina, Irene, Stella, Jane, Henry, Alex, Leo, Vakki 등. **빈도 적용 결과**: Vakki→박기 (46작품), karis kim→김카리스 (219), kris kim→김크리스 (69), johnny kim→김자니 (49), Suh Hee Sun→서희선 (42), jeremy yong→용제레미 (39), stone kim→김스톤 (33), EUN PYO HONG→홍은표 (21), denis lee→이데니스 (25), molly kim→김몰리 (19) — 총 500+ 작품 정확도 향상.
- **v11 (2026-05-11)**: 41,365 rows / **21 cols**. `artist_name_ko` 빈도 높은 의심 케이스 audit 후 fix. 사용자 지적 (남제에연/강섬메르/이인서브 — 원문 Nam Jeeyeon/Summer Kang/Lee In Seob)에서 시작해 dataset 의심 패턴 분석 → 다수 누락 음절 발견. 추가: `jee→지`, `seob/seop→섭`, `yeob/yeop→엽`, `hoon→훈`, `mook→묵`, `taek→택`, `gyeom→겸`, `yune→윤`, `reum→름`, `ram→람`, `woon→운`, `sook→숙`, `haeng→행`, `jeon→전` (4-char). Surname 보강: `leem→임`, `chon→천`, `cheon→천`. ENGLISH 사전 확장: Summer, Lydia, Lynn, Noel, Erin, Stone, Rain 등. Stage 4 외국인 음역도 `_convert_first_part` 사용 (영어 사전 적용). **수정 효과 dataset 검증**: Nam Jeeyeon→남지연 (20작품) / Summer Kang→강썸머 (10) / Lee In Seob→이인섭 (77) / lydia lee→이리디아 (126) / jihoon yang→양지훈 (83) / panmook kim→김판묵 (37) / byung taek jeon→전병택 (38) / Jaeyeon Leem→임재연 (28) — 총 700+ 작품 정확도 향상.
- **v10 (2026-05-11)**: 41,365 rows / **21 cols**. `estimated_ho` 컬럼 추가 — 한국 캔버스 호수 (F 타입). 기존 `visionai.price_engine.preprocessing.dimension_parser.area_to_ho_f()` 사용 (HO_F_TABLE 기반 `np.interp` 선형 보간). 표준 호수 매칭률 100% 검증 (60.6×50.0=12호, 72.7×60.6=20호, 116.8×91.0=50호, 162.2×130.3=100호 모두 정확). 분포: median 15.81호 / mean 27.5호 / max 200호. 호수 buckets: 0-5호 15.1% / 5-10호 22.2% / 10-20호 19.4% / 20-30호 13.5% / 30-50호 13.4% / 50-100호 10.6% / 100호+ 3.8%.
- **v9 (2026-05-11)**: 41,365 rows / **20 cols**. `artist_name_ko` 빈도 높은 작가 정확도 향상. 실제 dataset top 25 작가 (총 ~7,500 작품) sampling 후 분석 — 1088 작품 `gyobeom an → 안교어므` (실제 안교범) 등 빈도 높은 케이스 fix. (1) `gyob`/`kyob` 4-char 매핑 제거 — `gyo(3)+beom(4)` 분해가 정답이지만 `gyob(4)+eom(2)` 분해되어 잘못. (2) 추가 음절: `hyea/hyae→혜`, `ryang/lyang→량`, `tai/tay→태`, `youn→윤`, `beom→범`, `gab/gap→갑`, `oh→오`(2-char), `ok→옥`, `il→일`, `mee→미`, `hea→혜`. (3) surname 보강: `um→엄`, `gheem→김` (드문 표기). (4) `ENGLISH_FIRSTNAME_TO_KO` 확장: `mia→미아`, `lacey→레이시`, `karis→카리스`, `eury→유리` 등 한국계 영어 이름. **검증 44/44 (100%)** — top 25 작가 음역 모두 정확. (예외: `lim miryang → 임미량` user expected 임미향 — Mi-hyang vs Mi-ryang Romanization 표기 차이로 음역 logic 한계).
