...
> ⚠️ **핵심 framing**: A.4 = **Working hypothesis "cross-artist applicable signal 이 cold-start LAO 에 유효" 의 단순 일반화 반박**. Title text embedding 도 artwork-level cross-artist signal 인데 결과는 **저가 +1.22%p 악화 + 75/100 seeds violation = Axis A 최악**.
...
>
> **Working hypothesis 단순 일반화 반박** (provisional, 코덱스 framing 톤): A.3 (geometry) = strongest / A.4 (text embedding) = FAIL → "artwork-level cross-artist signal 일반이 cold-start LAO 에 유효" 가설 **반증** 또는 **혼재 결과**. A.3 의 effect 가 geometry 의 특수 효과 일 가능성 ↑ / text embedding 은 본 dataset (8,495) + PCA K=10 reduction 에서 informative signal 만들기 부족.
>
...
### 3.1 왜 A.4 가 FAIL 했나? (가설 수준)
- **PCA K=10 의 overfitting risk**: 384-dim → 10 reduction 후에도 8,495 sample 기준 dim ratio 가 새 features 중 가장 큼
- **Text embedding 의 noise dominance**: `Untitled` (164회) 등 generic title 이 noise driver / multilingual model 의 zero-shot 한계 (작품 가격 prediction 에 unfine-tuned)
- **Pretrained embedding 의 task mismatch**: paraphrase task 로 학습된 model 이 art price prediction 의미 capture X
- → A.4 features = **negative information** (signal-to-noise ratio < 0)
...
### 3.3 Cold-start LAO 평가 구조의 dim sensitivity (가설)
- 작은 sample (8,495) + cold-start (artist holdout) → high-dim feature 의 generalization 약함
- A.4 결과 = **dim 증가 자체가 cold-start LAO 에서 risk** 가설 시사 (provisional)
...
1. ✅ A.4 결과 보고 — 본 commit
2. ⏳ Deviation log: 본 cycle entry (none, 정상 흐름) + A.5 escalation 결정
3. ⏳ 코덱스 결과 검수 (선택)
4. ⏳ A.5 prereg freeze 진입 (image embedding, CLIP/ResNet — heaviest escalation, image fetch + storage 비용 ↑)
5. ⏳ (사용자 결정) **A.5 진입 가치 의문 — 사용자 의사결정 영역** (A.4 dim sensitivity 가설 적용 시 A.5 도 유사 FAIL 가능)

> ⚠️ **A.5 진입 의사결정 영역 (코덱스 권고 가능 영역)**:
> - **A.5 진입 시**: working hypothesis 의 image embedding 차원 confirmation
> - **A.5 보류 / Axis A 종료 시**: A.4 결과 + A.5 진입 ROI 의문 → Axis A 전체 종료 + Axis B (Phase A pre-screen) 또는 **A.3 단독 shadow 검토 (별도 의사결정 gate)** 직접 진입
...
| **A.4 결과 보고 검수 (예정)** | FAIL 정당성 + A.5 진입 ROI 평가 (working hypothesis 재고) |
