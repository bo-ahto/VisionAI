...
미술품 가격 결정에 관한 선행 연구로는 Rosen(1974)의 헤도닉 가격 모델이 있다:
...
동일 크기/매체라도 작가에 따라 가격이 크게 달라진다. 헤도닉 모델에서는 이를 작가 고정 효과 $\alpha_j$로 추정하지만, 본 모델에서는 **작가를 직접 식별하지 않고** 작가의 관측 가능한 속성(작품수, 팔로워, 전시 이력, 생년 등)을 피처로 사용한다.

GBDT는 이 피처들의 비선형 조합으로 작가 프리미엄을 암묵적으로 학습한다. 예를 들어, "작품수 150 + 팔로워 2,000 + 개인전 10회"인 작가와 "작품수 5 + 팔로워 0 + 개인전 0회"인 작가의 가격 차이를 트리 분기로 포착한다.
