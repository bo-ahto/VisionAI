...
> **문서 버전**: v3.1 (Codex 11회차 반영 — 게이트 기준 정합성 보강)
> **작성일**: 2026-03-26
...
### 1.4 피처 기여도 (Ablation 실측, SHAP 이론 기반 — Lundberg & Lee, 2017 [7])
...
헤도닉 가격 모델은 미술품 가격을 관찰 가능한 특성들의 함수로 분해하는 **경제학적 문제 정의 프레임워크**다 (Rosen, 1974 [1]). 반복 판매 모델(Mei & Moses, 2002 [10])은 시장 지수 추정에 활용되며, 본 엔진에서는 시장 컨텍스트 피처로 간접 반영한다.
본 엔진에서는 이 프레임을 "가격이 특성의 함수다"라는 문제 정의로 채택하되, 함수의 구체적 형태는 ML 모델(CatBoost)이 데이터에서 학습한다. 딥러닝 기반 대규모 벤치마크(Aubry et al., 2025 [8])와 정형 데이터 모델 비교 연구(Bento et al., 2024 [9])가 이 접근의 타당성을 뒷받침한다.
...
log 공간 예측을 원래 스케일로 복원할 때 주의가 필요하다.
  ...
  ⚠️ 구현 시 필수 검증 절차 (Sprint 1):
    1. holdout test set에서 가격대별 median(P̂/P_actual) 산출
       → 1.0 근처면 편향 없음, <0.9이면 체계적 과소추정
    2. 고가 구간(1억+) 별도 확인:
       → median(P̂/P_actual) < 0.85이면 Duan's smearing correction 적용
    3. 보정 적용 시 전/후 MAPE 비교하여 효과 확인
    4. 결과를 Sprint 1 평가 리포트에 "재변환 편향 검증" 섹션으로 포함
  ...
  보정 적용 판단 기준:
    ...
    │ < 0.85       │ smearing correction 필수 적용  │
    │ > 1.05       │ 과대추정 조사 (고가 이상치?)    │
    └──────────────┴──────────────────────────────┘
```
...
No  이름                  타입    변환 공식                                   근거
── ──────────────────── ────── ──────────────────────────────────────────── ──────
 1  artist_name          CAT    원본 작가명 (3,286명 + 특수 카테고리)                  헤도닉 모델 핵심 변수 [1][12]
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ 구현 주의: auction_date 부재에 따른 time proxy 한계
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  현재 데이터에 auction_date가 없어 session_number를 시간 proxy로 사용한다.
  이로 인해 아래 한계가 발생하며, 구현 시 반드시 인지해야 한다:

  한계 1 — 달력 기반 시즌성 포착 불가
    봄/가을 메이저 시즌, 연말 효과 등 "몇 월인가?"에 따른 가격 변동을 반영할 수 없다.
    → Phase 2에서 auction_date 확보 후 sale_month, sale_quarter 피처 추가로 해소.

  한계 2 — "최근 N건"의 실제 시간 간격 미반영
    artist_recent_avg_3이 "최근 3건"이지만, 3건이 1개월 이내인지 3년에 걸친 것인지 구분 불가.
    → artist_days_since_last를 회차 간격(gap)으로 근사하되, 실제 일수가 아님을 주석 명시.

  한계 3 — 타입 간 시간 정렬 불가
    위클리 회차 400과 메이저 회차 180이 실제로 같은 시기인지 알 수 없다.
    → auction_type별 독립 분석만 수행. 타입 간 시장 비교는 auction_date 확보 후에 한정.

  대응 전략:
    1. Phase 1: session_number proxy로 진행 (현재 baseline이 이 조건에서 MAPE 40.17% 달성)
    2. Sprint 1에서 time proxy 타당성 검증:
       a) 같은 타입 내에서 회차가 단조증가하는지 확인 (번호 정렬)
       b) 가능하다면 경매 제목/메타에서 날짜 힌트를 추출하여 회차↔실제 날짜 대응 샘플 확보
          → 샘플 20건 이상에서 회차 순서와 날짜 순서의 일치율 확인
          → 일치율 < 90%이면 time proxy 신뢰도 경고 문서화
       c) 역전 케이스가 있으면 개수 보고 + 해당 건 제외 여부 결정
       ⚠️ 이 검증은 proxy의 "완벽한 입증"이 아니라 "치명적 오류 부재 확인" 수준이다.
          auction_date 확보 전까지는 근본적 한계가 남음을 인정한다.
    3. auction_date 확보를 Phase 2 전환 게이트의 권장 조건으로 추가
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ⚠️ 구현 주의: Cold Start / D등급 slice 평가 (필수)
  ─────────────────────────────────────────────────
  D등급(신규/미상 작가)은 전체의 약 46%를 차지하므로 무시할 수 없다.
  이 그룹에서 모델이 "추정가 × 고정 할인율"로 수렴하는지,
  그마저도 작동하지 않는 구간이 있는지 반드시 별도 분석해야 한다.
  ...
  필수 검증:
    ...
    3. D등급 예측의 "가격 범위 폭"이 과도하게 넓지 않은지 확인
       ...
       → 너무 넓으면 "예측 불가" 표시가 더 정직할 수 있음
    4. D등급 fallback 대체값의 품질:
       → estimate_tier + auction_type 그룹 평균이 실제 가격과 얼마나 가까운지
       → 그룹 크기가 너무 작으면(<10건) 대체값 자체가 불안정

  Sprint 1.5 산출물:
    D등급 별도 리포트 — 위 4개 항목 결과 포함
    → 결과에 따라 D등급 UX 결정:
       Option A: 넓은 범위 + "참고용" 면책
       Option B: "데이터 부족으로 예측 불가" 표시
       Option C: 추정가 범위만 표시 (모델 예측 미사용)
...
| 4 | **유찰 데이터 부재** — 선택 편향 | 낮음 | "낙찰 기준" 명시. 유찰 데이터 확보 시 Heckman IMR 보정 (Cogent Economics, 2018 [11]) |
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  4. Prediction Interval Backtest
     Phase 1: 고정 margin 방식(5.3절 방법 1)의 grade별 실제 coverage 검증.
       → A등급 ±20% 구간에 실제 낙찰가의 몇 %가 포함되는지 측정.
       → 각 grade의 margin이 적절한지 판단 (과소→margin 확대, 과대→margin 축소).
     Phase 2: Q10-Q90 분위수 회귀(5.3절 방법 2)의 80% coverage 달성 검증.
       → holdout에서 실제 낙찰가가 [Q10, Q90] 구간에 80%+ 포함되는지 확인.
...
Sprint 1 — Baseline 재현 + 최소 피처 CatBoost
  ...
  □ 회차 순서 = 시간 순서 가정 검증 (타입별 단조증가 확인)
  □ 재변환 편향 검증:
    - 가격대별 median(P̂/P_actual) 산출
    - 1억+ 구간 별도 확인 (< 0.85이면 smearing correction)
    - 보정 전/후 MAPE 비교
  □ 세그먼트별 MAPE 리포트 생성
...
Sprint 1.5 — 정적 JSON + 테스트 페이지 + 검증 (9장 항목 반영)
  ...
  □ Cold Start / D등급 slice 별도 리포트 (5.2절 필수 검증 4개 항목)
  □ D등급 UX 결정 (Option A/B/C 중 선택)
  □ 주요 작가 10명 수동 검증
...
```
  ...
  □ D등급 품질 하한 충족:
    - D등급 MdAPE 보고 완료
    - D등급 UX 결정 완료 (Option A/B/C) + 결정 근거 문서화
    - D등급 MdAPE > 100%이면 해당 하위 그룹은 "예측 불가" 처리 필수
  □ 재변환 편향 검증 통과:
    - 모든 가격대에서 median(P̂/P) ≥ 0.95 (정상 범위)
    - 0.85~0.95 구간이면 smearing correction 적용 후 재검증하여 ≥ 0.95 달성
    - 0.85 미만 구간이 존재하면 Phase 전환 불가 (2.2절 판단 기준표와 정렬)
  ○ (권장) auction_date 확보 — 미확보 시 time proxy 한계 문서화 후 진행 가능
```
...
```
이 PR은 K-Auction 경매 데이터 기반 가격 예측 엔진의 상세 기획서입니다 (v3.1).
Codex 12회차 종합 리뷰를 반영한 v3.1 버전입니다. 아래 관점에서 검토해주세요:
