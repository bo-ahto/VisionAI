...
프론트는 작가 이름 검색 후 `artist_key`를 고르는 방식이며, 주기적으로 추가 데이터를 학습해
모델 버전을 올린다.
...
책임·의존성 중심. 상세 I/O는 §2(데이터 계약)·§9(API)를 참조.
...
각 컴포넌트는 단일 책임을 가지며, 인터페이스(§2/§9)로만 통신해 독립 테스트가 가능해야 한다.
...
- guard 테스트로 검증(§11): Cold 실행 중 history/search 의존성 호출 시 실패.
...
### 7.2 Cold 학습 파이프라인 (k80)

```text
[학습 데이터: 가격 라벨 + 작품 피처 + 입력가능 작가 메타]
...
### 7.3 하이브리드 재학습 (고정 주기 + 성능 게이트)

```text
[트리거: 고정 주기 도래]  (운영 합의 주기, 예: 분기)
        |
        v
[데이터 cutoff 고정 → 신버전 학습(7.1 / 7.2)]
        |
        v
...
1. **Cold 번들 동결** — k80 기준안을 `cold_k80_conservative_official_v0.1_candidate`로 단일 `runtime_store.joblib` 화.
2. **manifest/schema 정의** — Warm/Cold manifest(§3.3), feature schema, enum 동결.
3. **Predictor adapter** — `predict(input)->output`(§2.5)로 Warm/Cold predictor 래핑, DB/CSV/검색 차단.
4. **Registry loader** — 버전 디렉토리 + active 포인터 로드/검증(§3.4), 스왑/롤백 API.
5. **Router** — Warm 술어(§6.1) + 실패 모드(§6.2) 구현, strict-Cold 코드 강제(§6.3).
6. **API 서비스** — §9 엔드포인트, 기동 시 두 번들 1회 로드.
7. **로깅/모니터링** — §10 스키마 적재.
8. **parity 테스트** — Warm 607 / Cold 3,099 fixed test diff 0 검증.
9. **integration / strict-Cold guard / invariant 테스트** — §11.
10. **shadow deploy** — 실트래픽 복제, 신/구 diff 로깅.
11. **포인터 승격** — 성능 게이트(§8.2) 통과 시 active 포인터 교체.
...
같은 번들 계약·parity·strict-Cold 강제·모니터링으로 동결하면 운영 가능하다. 재학습은 새 버전 아티팩트로
만들고, 성능 게이트·shadow 통과 후 active 포인터만 교체한다.
