...
### H73. Cold 위험도는 크기/호수 극단값으로 구분할 수 있을 것이다

- 배경
- Cold 전체 가격 범위는 `x2.27`로 넓어 단일 가격 서비스에 부담이 큼
- 질문
- 대형 호수, 초대형 면적, 면적 상위 구간을 이용하면 저위험/고위험이 나뉘는가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- train 기준 크기 threshold와 H32 Cold 예측을 사용함
- 내부 Cold calibration에서 80% 가격 범위 폭을 계산하고 Cold test에 적용함
- 검증 결과
- 크기 low-risk: `x2.14`, coverage `0.875`, median APE `0.2346`
- 크기 high-risk: `x2.88`, coverage `0.794`, median APE `0.4448`
- 결론
- 고위험 탐지에는 유효함
- low-risk도 폭이 충분히 좁아지지는 않음

### H74. Cold 위험도는 2D/3D와 크기 안정성으로 구분할 수 있을 것이다

- 배경
- H32에서 3D 조건부 fallback이 Cold 성능을 개선했음
- 질문
- 2D/3D와 크기 안정성 기준으로 더 안정적인 Cold subset을 만들 수 있는가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- 표준 2D와 표준 3D를 크기 극단값 제외 기준으로 정의함
- 각 그룹의 calibration 폭, coverage, median APE를 비교함
- 검증 결과
- 표준 3D: `x2.06`, coverage `0.887`, median APE `0.2094`
- 표준 2D: `x2.42`, coverage `0.783`, median APE `0.3767`
- 결론
- 표준 3D는 Cold 내 가장 안정적인 후보임
- 표준 2D는 여전히 위험함
- 표준 3D도 단일 가격 서비스 수준으로 좁지는 않음

### H75. Cold 위험도는 재료/바탕 희소성으로 구분할 수 있을 것이다

- 배경
- 재료/바탕 정보는 운영에서 만들 수 있는 입력 변수임
- 질문
- 흔한 재료/바탕과 희소 재료/바탕이 Cold 오차 폭을 다르게 만드는가
- 현재 판단
- 검증 완료
- 미채택
- 연구 방법
- train에서 일정 횟수 이상 등장한 재료/바탕을 common으로 정의함
- common과 rare 그룹의 calibration 폭과 오차를 비교함
- 검증 결과
- common material 그룹이 Cold test의 거의 전체를 차지함
- common material 폭 `x2.26`으로 전체 Cold `x2.27`과 거의 같음
- 결론
- 재료/바탕 희소성만으로는 Cold 위험도 구분 효과가 없음

### H76. Cold 위험도는 예측 가격대별로 달라질 것이다

- 배경
- 예측 가격이 너무 낮거나 높은 구간은 오차가 커질 수 있음
- 질문
- 예측가 중간 구간은 극단 구간보다 좁은 가격 범위가 가능한가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- 내부 calibration 예측값의 하위 20%, 중간 60%, 상위 20% 기준을 만듦
- 같은 기준을 Cold test 예측값에 적용함
- 검증 결과
- 중간 60%: `x2.16`, coverage `0.858`, median APE `0.2486`
- 하위/상위 20%: `x2.55`, coverage `0.857`, median APE `0.3194`
- 결론
- 예측가 극단 구간은 더 위험함
- 하지만 중간 구간도 폭 축소 효과는 제한적임

### H77. Cold 위험도는 면적-호수 불일치로 구분할 수 있을 것이다

- 배경
- 같은 호수라도 실제 면적이 다르면 가격 구조가 달라질 수 있음
- 질문
- 면적-호수 불일치가 작은 작품은 더 안정적인가
- 현재 판단
- 검증 완료
- 미채택
- 연구 방법
- `ho_area_gap_abs` train 기준 상위 구간을 위험 후보로 정의함
- 일관/불일치 그룹의 calibration 폭과 오차를 비교함
- 검증 결과
- 일관 그룹은 `x2.33`으로 전체보다 오히려 넓음
- 불일치 그룹은 coverage `0.749`로 기준 미달이며 median APE `0.3294`
- 결론
- 면적-호수 불일치 단독 기준은 안정적인 저위험 구분에 부적합함

### H78. 여러 위험 기준을 합친 risk score가 단일 기준보다 안정적일 것이다

- 배경
- 단일 기준보다 크기, 일관성, 재료, 예측가를 함께 보는 것이 더 안정적일 수 있음
- 질문
- 복합 risk score가 high-risk 작품을 더 잘 구분하는가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- 대형 호수, 초대형 면적, 크기-호수 불일치, 예측가 극단, 희소 재료/바탕을 점수화함
- 복합 low-risk와 high-risk를 나눠 비교함
- 검증 결과
- 복합 low-risk: `x2.17`, coverage `0.880`, median APE `0.2486`
- 복합 high-risk: `x2.83`, coverage `0.811`, median APE `0.4557`
- 결론
- high-risk 탐지는 유효함
- low-risk 가격 범위 축소 효과는 제한적임

### H79. 크기/호수 이상치 flag가 큰 오차 작품을 사전에 잡아낼 수 있을 것이다

- 배경
- Cold 큰 오차는 대형/극단 크기 작품에서 자주 발생함
- 질문
- 면적/호수 불일치 상위 10% 또는 면적 상위 10% 작품은 경고 대상으로 볼 수 있는가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- train 기준 극단 threshold를 사용해 이상치 flag를 만듦
- 해당 그룹의 calibration 폭과 오차를 확인함
- 검증 결과
- 극단 이상치: `x3.00`, coverage `0.865`, median APE `0.4313`
- 결론
- 이상치 flag는 신뢰도 경고 후보로 적합함
- 가격 예측 대상에서 별도 주의가 필요함

### H80. 조건을 만족하는 Cold subset만 서비스 후보로 두면 전체 Cold보다 실용적인 출력이 가능할 것이다

- 배경
- Cold 전체를 모두 같은 수준으로 서비스하기 어렵다면 일부 안정 구간만 제한적으로 제공할 수 있음
- 질문
- 운영 가능한 기준으로 선택한 Cold subset은 전체 Cold보다 실용적인가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- 복합 low-risk 또는 표준 3D를 선택적 서비스 후보로 정의함
- 전체 Cold와 가격 범위 폭, coverage, median APE를 비교함
- 검증 결과
- 선택적 서비스 후보: n `2,722`, 비중 `0.712`
- 가격 범위 `x2.10`, coverage `0.881`, median APE `0.2222`
- 전체 Cold: `x2.27`, coverage `0.855`, median APE `0.2786`
- 결론
- 선택적 subset은 전체 Cold보다 낫지만 여전히 범위가 넓음
- Cold는 정확한 단일 가격이 아니라 참고 추정가 + 낮은 신뢰도 정책으로 유지해야 함

### H81. 예측값을 train 가격 범위 안으로 clipping하면 Cold tail risk가 줄어들 것이다

- 배경
- Cold는 p95/p99 오차가 커서 가격 범위가 넓어짐
- 질문
- 예측값을 train 가격 분포의 상하한 안으로 자르면 큰 오차가 줄어드는가
- 현재 판단
- 검증 완료
- 미채택
- 연구 방법
- train 내부 Cold calibration에서 clipping 후보를 선택함
- release Cold test에서 p95, p99, median APE, q80 가격 배수를 검증함
- 검증 결과
- 단순 clipping은 shrink 정책보다 강한 후보로 남지 못함
- 결론
- 단순 clipping은 최종 후보로 부적합함

### H82. high-risk 작품의 예측값을 전체 중앙값 쪽으로 shrink하면 큰 오차가 줄어들 것이다

- 배경
- high-risk 작품은 예측값이 과도하게 튀면서 p95 오차를 키울 수 있음
- 질문
- high-risk 예측값을 전체 중앙값 쪽으로 일부 당기면 tail risk가 줄어드는가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- large_size, combined_score high-risk 조건에 대해 shrink weight를 calibration에서 선택함
- release Cold test에서 tail 개선과 median 손실을 확인함
- 검증 결과
- 복합 high-risk 중앙값 shrink 0.40:
- p95 `1.4860 -> 1.2226`
- median `0.2786 -> 0.2828`
- q80 가격 배수 `x2.00 -> x2.13`
- 결론
- p95 개선은 크지만 가격 범위 폭은 줄지 않음
- tail-risk 보정 후보로만 유지함

### H83. high-risk 작품의 예측값을 재료/호수 그룹 중앙값 쪽으로 shrink하면 큰 오차가 줄어들 것이다

- 배경
- 전체 중앙값보다 재료/호수 그룹 중앙값이 작품 구조를 더 반영할 수 있음
- 질문
- 그룹 중앙값 shrink가 tail risk와 median APE를 함께 개선하는가
- 현재 판단
- 검증 완료
- 부분 채택
- 연구 방법
- high-risk 작품에 대해 재료/호수 그룹 중앙값 방향으로 0.10~0.30 shrink를 적용함
- 후보 선택은 내부 calibration 기준으로 진행함
- 검증 결과
- high-risk 재료 중앙값 shrink 0.30:
- median `0.2786 -> 0.2739`
- p95 `1.4860 -> 1.2961`
- q80 가격 배수 `x2.00 -> x2.08`
- 결론
- 가장 균형적인 tail 보정 후보임
- 단, 가격 범위 폭은 줄지 않아 운영 확정 전 추가 검증 필요

### H84. high-risk 작품에만 예측값 상하한 cap을 적용하면 큰 오차가 줄어들 것이다

- 배경
- high-risk 작품에서 극단 예측값을 제한하면 tail이 줄어들 수 있음
- 질문
- high-risk 작품에만 cap을 적용하는 것이 shrink보다 나은가
- 현재 판단
- 검증 완료
- 미채택
- 연구 방법
- high-risk mask별로 train target quantile cap을 적용함
- 내부 calibration에서 후보를 선택하고 release test에서 확인함
- 검증 결과
- cap 후보는 최종 선택 후보에서 shrink보다 열세였음
- 결론
- cap 방식은 미채택

### H85. Cold tail risk는 모델 보정보다 신뢰도 경고와 함께 관리하는 것이 더 적절할 것이다

- 배경
- tail 보정으로 p95를 줄여도 사용자가 보는 가격 범위가 줄지 않을 수 있음
- 질문
- Cold tail risk는 보정만으로 해결 가능한가, 아니면 경고 정책이 필요한가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- p95 개선, median APE, q80 가격 배수를 함께 비교함
- 검증 결과
- p95는 개선 가능
- q80 가격 배수는 줄지 않음
- 결론
- Cold에는 보정 정책과 별도로 신뢰도 경고가 계속 필요함

### H86. tail 보정은 p95 개선과 median APE 손실을 함께 보는 기준이 필요하다

- 배경
- p95만 줄이는 후보는 일반 작품의 예측 품질을 해칠 수 있음
- 질문
- tail 보정 채택에는 어떤 기준이 필요한가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- p95, p99, median APE, within-30%, q80 가격 배수를 함께 확인함
- 검증 결과
- p95가 개선되어도 q80 가격 배수가 커질 수 있음
- median APE가 소폭 좋아져도 가격 범위 폭이 줄지 않을 수 있음
- 결론
- tail 보정은 단일 지표가 아니라 `p95 개선 + median 유지 + q80 폭 악화 제한`을 함께 봐야 함

### H87. 작가 이력 등급별로 가격 범위를 나누면 일부 Warm 구간의 폭을 줄일 수 있을 것이다

- 배경
- Warm은 작가 학습 이력 수에 따라 오차가 크게 달라짐
- 질문
- 작가 이력 등급별로 가격 범위를 다르게 주면 일부 구간의 폭을 줄일 수 있는가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- H66 Warm 모델 기준으로 내부 calibration에서 등급별 80% 폭을 계산함
- release Warm test에서 coverage와 오차를 확인함
- 검증 결과
- A등급 `x1.26`, coverage `0.813`, median APE `0.0603`
- B등급 `x1.31`, coverage `0.826`, median APE `0.0694`
- D등급 `x1.94`, coverage `0.794`, median APE `0.1568`
- 결론
- A/B는 좁은 범위가 가능함
- D는 넓은 범위와 신뢰도 경고가 필요함

### H88. Warm low-risk subset은 전체 Warm보다 좁은 가격 범위로도 coverage를 유지할 수 있을 것이다

- 배경
- Warm 전체 범위 `x1.52`를 모든 작품에 적용하면 안정 구간에는 불필요하게 넓을 수 있음
- 질문
- 복합 low-risk 구간은 전체보다 좁은 범위로도 coverage를 유지하는가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- A/B 등급, 크기 안정, 중간 예측가 구간을 조합해 low-risk를 정의함
- 내부 calibration 폭을 release Warm test에 적용함
- 검증 결과
- 복합 low-risk `x1.30`
- coverage `0.823`
- median APE `0.0617`
- 결론
- Warm low-risk에는 좁은 가격 범위를 줄 수 있음

### H89. Warm high-risk subset은 별도 넓은 범위와 신뢰도 경고가 필요할 것이다

- 배경
- 저이력 작가나 위험 구간은 전체 Warm보다 오차가 클 수 있음
- 질문
- Warm high-risk에는 별도 넓은 범위와 경고가 필요한가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- D등급, 대형/극단, 예측가 극단 구간을 high-risk 후보로 비교함
- 검증 결과
- D등급 `x1.94`, p95 APE `2.2005`
- 복합 high-risk `x1.61`, p95 APE `1.2834`
- 결론
- Warm high-risk는 신뢰도 경고가 필요함

### H90. Warm high-risk 예측값 shrink로 전체 가격 범위 폭을 줄일 수 있을 것이다

- 배경
- Cold에서는 high-risk shrink가 p95를 줄였으므로 Warm에도 적용 가능성을 확인함
- 질문
- Warm high-risk 예측값을 등급/재료 중앙값 쪽으로 당기면 전체 폭이 줄어드는가
- 현재 판단
- 검증 완료
- 미채택
- 연구 방법
- 내부 calibration에서 shrink 후보를 선택하고 release Warm test에 적용하려 함
- 검증 결과
- calibration 기준을 통과한 shrink 후보가 없음
- 결론
- Warm은 shrink 보정보다 구간별 가격 범위 출력이 적절함

### H91. Warm은 모델 교체보다 구간별 출력 정책으로 가격 범위를 줄이는 것이 적절할 것이다

- 배경
- Warm 모델 자체는 H66으로 안정적이므로 출력 정책이 더 효과적일 수 있음
- 질문
- 모델 보정 없이 구간별 가격 범위만으로 서비스 해석을 개선할 수 있는가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- 전체 단일 범위와 등급별 범위를 비교함
- 검증 결과
- 전체 `x1.52`
- A/B/low-risk `x1.26~x1.31`
- D `x1.94`
- 결론
- 등급별 출력 정책이 더 실용적임

### H92. Warm 가격 범위 정책도 별도 holdout 또는 CV로 최종 재검증해야 한다

- 배경
- release test를 여러 실험에서 반복 사용했음
- 질문
- Warm 가격 범위 정책을 운영 확정하기 전 추가 검증이 필요한가
- 현재 판단
- 검증 완료
- 채택
- 연구 방법
- 현재 결과의 검증 강도와 release test 반복 사용 리스크를 함께 평가함
- 검증 결과
- 등급별 범위 방향성은 명확하지만 release test 반복 사용 리스크가 있음
- 결론
- 최종 운영 전 별도 holdout 또는 CV 재검증 필요
...
- H73~H80은 Cold 저위험/고위험 구분 가능성을 확인했고, high-risk 탐지는 가능하지만 low-risk도 가격 범위가 `x2.06~x2.17` 수준으로 넓어 단일 가격 서비스 수준은 아님을 확인함
- H81~H86은 Cold tail risk 보정 가능성을 확인했고, p95는 줄일 수 있지만 가격 범위 폭은 줄지 않아 신뢰도 경고가 계속 필요함을 확인함
- H87~H92는 Warm 가격 범위 폭 축소 가능성을 확인했고, A/B/low-risk는 `x1.26~x1.31` 수준으로 줄일 수 있지만 D/high-risk는 넓은 범위와 경고가 필요함을 확인함
