...
| 가격 범위 | H70 내부 calibration 기반 조건별 범위 | Warm 전체 `x1.52`, Cold 전체 `x2.27` | 운영 검토 후보, 서비스 확정 아님 |
...
- H70 내부 calibration split 기준으로 조건별 가격 범위 방향성은 유지 가능함
- 단, coverage 수치만으로 서비스 가능 여부를 판단하면 안 됨
- 가격 범위는 실제 사용자에게 보이는 폭이 넓으면 “맞는 범위”여도 실용성이 떨어질 수 있음
- 현재 가격 범위는 “바로 운영 확정”이 아니라 “운영 검토 후보”로 관리함

### 가격 범위 서비스 가능성 판단 기준

- coverage
- 목표한 80% 범위가 실제 가격을 약 80% 이상 포함하는지 확인
- 낮으면 가격 범위가 실제 가격을 자주 놓치므로 서비스 위험이 큼
- 현재 H70 기준 Warm 전체 `0.821`, Cold 전체 `0.855`로 전체 coverage는 목표에 근접함

- 범위 폭
- 예측가 대비 가격 범위가 너무 넓으면 사용자가 해석하기 어려움
- 예: `x2.27`은 예측가가 1,000만 원일 때 대략 440만~2,270만 원 수준의 넓은 범위로 해석됨
- Cold 고위험 구간은 `x2.88~x3.11`까지 필요해 단순 가격 범위보다 “낮은 신뢰도” 표시가 더 중요할 수 있음

- 구간별 안정성
- 전체 coverage가 좋아도 특정 구간이 낮으면 운영 위험이 남음
- Warm D등급, Cold 표준 2D, Cold 대형/초대형 구간은 별도 경고 후보로 유지

- 운영 재현성
- 가격 범위 폭은 test residual로 직접 정하면 안 됨
- H70처럼 내부 calibration split에서 폭을 정하고, test에서는 검증만 해야 함
- production pipeline에서도 동일한 calibration split 또는 재현 가능한 calibration 절차가 필요함

- 현재 판단
- 가격 범위는 “단일 가격만 제공하는 것보다 안전한 출력 방식”이라는 방향성은 유효함
- 그러나 현재 수치는 서비스 확정 지표가 아니라 운영 UX와 위험 문구를 정하기 위한 검토 지표임
- 서비스 적용 전에는 가격 범위 폭, 구간별 coverage, 경고 문구 기준을 함께 확정해야 함

### Warm 가격 범위 후보

| Warm 구간 | 80% 가격 범위 배수 | test coverage | 판단 |
|---|---:|---:|---|
| 전체 | x1.52 | 0.821 | 운영 검토 후보 |
...
- H73-H80에서 Cold 위험도 기준을 추가로 검증함
- 기준은 test 오차를 보고 만들지 않고 train/internal calibration 기준으로만 정의함
- 가장 안정적인 후보
- 표준 3D: `x2.06`, coverage `0.887`, median APE `0.2094`
- 선택적 서비스 후보: `x2.10`, coverage `0.881`, median APE `0.2222`
- 고위험 후보
- 크기 high-risk: `x2.88`, median APE `0.4448`
- 복합 high-risk: `x2.83`, median APE `0.4557`
- 극단 이상치: `x3.00`, median APE `0.4313`
- 판단
- 저위험/고위험 구분은 가능함
- 다만 저위험 후보도 `x2.06~x2.17` 수준으로 여전히 넓음
- Cold를 정확한 단일 가격 서비스로 쓰기 어렵다는 판단은 유지함
- Cold는 참고 추정가, 넓은 가격 범위, 낮은 신뢰도 경고 중심으로 운영 검토함

### Cold tail risk 완화 추가 검증

- H81-H86에서 큰 오차를 줄이는 후처리 정책을 추가 검증함
- 정책 선택은 test가 아니라 내부 Cold calibration 기준으로 진행함
- 가장 균형적인 후보
- high-risk 재료 중앙값 shrink 0.30
- median APE `0.2786 -> 0.2739`
- p95 APE `1.4860 -> 1.2961`
- p99 APE `3.6093 -> 3.4972`
- q80 가격 배수 `x2.00 -> x2.08`
- 판단
- tail risk는 줄일 수 있음
- 하지만 80% 가격 범위 폭은 줄지 않고 오히려 소폭 커짐
- 따라서 이 방식은 가격 범위를 좁히는 정책이 아니라 극단 오차 완화 보정 후보임
- 운영 확정 전에는 별도 holdout 또는 CV에서 추가 재검증해야 함
