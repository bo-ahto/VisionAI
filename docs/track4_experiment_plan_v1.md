...
|---|---|---|---:|---|
| T4-H0 | 데이터셋 재구성 | 파편화된 1차 시장 데이터를 공통 schema로 통합한 뒤 학습 가능한 기준 데이터셋을 만들 수 있다 | 0 | 진행 중 |
| T4-C1 | 가격 클렌징 | 가격 하한/상한과 통화/환산 검증으로 target 오류를 줄일 수 있다 | 0 | 예정 |
| T4-C2 | 크기 클렌징 | 크기 파싱 오류를 제거하면 면적/호수 피처의 신뢰도가 높아진다 | 0 | 예정 |
| T4-C3 | 재료/지지체 클렌징 | 원본 재료 문자열 기준 재분류로 운영 가능한 재료 피처를 만들 수 있다 | 0 | 예정 |
| T4-C4 | 작가명 클렌징 | 작가명 표준화와 동명이인 후보 점검이 Warm/Cold split 오류를 줄인다 | 0 | 예정 |
| T4-C5 | 중복 클렌징 | 동일 작품 중복을 flag로 관리하면 학습 데이터 편향을 줄일 수 있다 | 0 | 예정 |
