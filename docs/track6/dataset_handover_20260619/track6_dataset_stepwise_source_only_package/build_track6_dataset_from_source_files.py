#!/usr/bin/env python3
"""Shared helpers for building a Track6 dataset from source CSV files.

이 파일은 직접 실행하는 진입점이 아니라, 단계별 실행 파일들이 import해서 쓰는
공통 함수 모음이다. 공유받는 사람은 목적에 따라 아래 파일을 실행한다.

    - 새 자동 split 생성: run_all_auto_steps.py
    - 기존 frozen split 재현: run_all_frozen_steps.py
    - 각 단계 개별 실행: step_01_*.py ~ step_08*.py
...
기존 기준 frozen split을 재현하려면:
    `02_frozen_reference/track6_split_membership.csv`와
    `02_frozen_reference/track6_identity_overrides.csv`를 넣고
    `run_all_frozen_steps.py` 또는 `step_06b_create_frozen_split.py`를 실행한다.

새 split을 생성하려면:
    frozen reference 없이 `run_all_auto_steps.py` 또는
    `step_06a_create_auto_split.py`를 실행한다.
"""
...
def create_working_splits_from_auto_policy() -> None:
    ...
    이 경로는 기존 성능표 재현용이 아니다. 현재 원본 CSV와 현재 split 정책으로
    새 train/validation/test dataset을 만든다. 기존 frozen split을 재현하려면
    `create_working_splits_from_frozen_membership()`를 호출하는
    `step_06b_create_frozen_split.py`를 사용한다.
    """
...
def main() -> int:
    print(
        "이 파일은 공통 함수 파일입니다.\n"
        "새 자동 split은 `python3 run_all_auto_steps.py`를 실행하고,\n"
        "기존 frozen split 재현은 `python3 run_all_frozen_steps.py`를 실행하세요.",
        file=sys.stderr,
    )
    return 2
