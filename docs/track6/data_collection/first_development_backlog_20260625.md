# Track6 데이터 수집/표준화/운영 1차 개발 백로그
...
선행 조건이 Epic 단위(예: `E3/E4`)로 적힌 항목은 **해당 Epic의 P0 및 (gate) Task 완료**를 의미한다(검증 Epic이 화면 Epic에 의존하는 경우 gate 화면 완료를 포함). Task 단위 의존이 명확한 경우는 Task ID로 적는다.
...
| E0-T02 | OpenAPI 초안 작성 | P0 | API 문서 | `openapi/track6_data_collection_v1.yaml` | request/response/status/error/pagination 반영 | schema lint |
...
| E2-T05 | Print Bakery raw 적재 연결 | P0 | E2-T02 | Print Bakery collector DB path | Cafe24/API/HTML fallback 적재 | fixture run |
| E2-T06 | Artsy DB writer/backfill 연결 | P0 | E2-T02 | Artsy adapter | 기존 CSV/export 결과 적재 | backfill test |
| E2-T07 | Saatchi DB writer/backfill 연결 | P0 | E2-T02 | Saatchi adapter | 기존 CSV/export 결과 적재 | backfill test |
...
| E4-T06 | 신규 작가 후보 큐 생성 | P0 | E4-T04 | new artist queue view/job | 승인 전 artist_key 미생성 | query test |
...
| E5-T07 | model registry 구현 | P1 | E5-T05 | model registration | snapshot/model_version 연결 | API/DB test |
| E5-T08 | model deployment 구현 | P1 | E5-T07 | deployment service | active/rollback 전이 | transition test |
| E5-T09 | prediction log 구현 | P1 | E5-T08 | log writer | model/deployment/route 추적 가능 | API test |
...
| E6-T04 | public artist/search/candidate API | P0 | E4-T04, E4-T06 | public API | 원천 정보 비노출 | schema/API test |
| E6-T05 | public price prediction API | P0 | E5-T08, E6-T04, E0-T06 | prediction endpoint | model/as_of/card 포함 | API test |
...
| E7-T03 | 신규 작가 후보 화면 | P1 | E6-T04 | candidate form | artist_key 즉시 생성 없음 | UI/API test |
...
| E7-T09 | 신규 작가 후보 큐 | P1 (gate) | E6-T07 | review screen | approve/recheck/hold/reject 처리 | UI/API test |
...
| E9-T09 | 장애 격리 검증 (NFR-02) | P1 (gate) | E6-T05 | isolation test report | 수집 DB 비가용 시에도 예측 API가 승인 snapshot/feature store/model bundle로 정상 응답 | fault injection test |
| E9-T08 | 운영 전환 체크리스트 | P1 (gate) | E9-T01~T07, E9-T09 | launch checklist | PRD 성공 기준 충족 | final review |
...
### M1 구성 Task (Art1 1원천 · 작가명 검수 큐 1종 · 최소 화면)

- 기반: E0-T01~T04, E0-T06, E0-T07, E0-T08, E1-T01~T08
- 수집: E2-T01, E2-T02, E2-T03, E2-T04 (Art1만)
- 표준화: E3-T01~T04 (Art1만)
- 작가 identity: E4-T01~T07
- snapshot/model: E5-T01~T05, E5-T07~T09
- API: E6-T01~T05, E6-T06(수집 대시보드), E6-T07(작가명 큐), E6-T08
- 화면: E7-T01, E7-T02, E7-T04, E7-T07

**M1 의존 닫힘 규칙**: 위 목록은 M1 spine이다. (1) 여기 나열된 Task의 **직접 선행 Task는 M1에 함께 포함**된다(예: E2-T03, E0-T03). (2) 다중 원천 Task(E3-T01~T04 parser, E6-T06 대시보드)는 **Art1 경로로만 구현·검증**하고, 그 Task의 Epic 단위 선행(`E2/E3`)은 M1에서는 Art1 관련 Task로만 해석한다. 나머지 원천 확장은 M2에서 같은 Task로 완성한다. (3) 따라서 M1에는 M2 전용 Task(E3-T07 등)에 대한 의존이 없다. (4) 집계 품질감사(E3-T07)는 M2다. M1의 수집 대시보드(E7-T04)·snapshot 요약(E5-T01)은 normalized quality flag 기반 **기본 표시**까지로 한정하고, 보유율/성공률 등 집계 지표는 M2에서 보강한다.

M1 완료 = Art1 데이터로 사용자 예측 화면이 active deployment 기준 응답(가격/신뢰도/as_of/1차 시장 카드)을 표시하고, 어드민이 화면에서 작가명 검수 1건을 처리한다(로드맵 §5.5 M1 완료 기준). freshness 경고/카드 숨김은 M2.

### M2 구성 Task (폭 확장)

- 나머지 원천: E2-T05~T08, E3-T05~T07, E4-T08
- 검수 큐/freshness 전체: E5-T06, E6-T09/T10/T11, E7-T03/T05/T06/T08/T09/T10/T11/T12
