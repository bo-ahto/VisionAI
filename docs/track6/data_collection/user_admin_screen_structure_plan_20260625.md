...
snapshot 반영은 2단계 권한 분리이며, API의 2개 액션과 1:1로 매핑된다.
...
- 운영자의 "확정 요청"과 데이터 관리자의 "생성 승인"은 별도 액션이며, 한 사람이 두 단계를 동시에 끝내지 않는다. 화면의 두 버튼은 API의 두 액션(확정 요청, 생성 승인)과 1:1로 대응한다.
