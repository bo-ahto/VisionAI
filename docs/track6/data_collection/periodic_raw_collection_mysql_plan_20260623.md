...
| `approved_at` | 승인 시각 |
| `approval_idempotency_key` | 생성승인 멱등 키(요청 `idempotency_key`와 구분). **전역 UNIQUE**(요청 `idempotency_key`와 동일 정책, §5.16). API 생성승인의 멱등 키가 여기 저장·replay되어 같은 승인이 중복 생성을 일으키지 않게 한다. 같은 키 + 다른 payload 호출은 충돌로 거부한다 |
