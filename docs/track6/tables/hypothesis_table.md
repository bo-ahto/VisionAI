...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| T6-H1 | T6-G1 | strict cold와 Stable Warm 기준을 적용한 Track6 split이 최종 보고 기준으로 더 적합할 것이다 | Track3/4/5 방법을 반영해 클렌징 후보를 확정하고 validation/test 우선 split으로 한글명, 동명이인, Stable Warm train 작품 수, Cold 이름 중복, 작가당 평가 작품 수를 검증 | Track6 split | split metadata | Track5 split | Cold 이름 중복 0, Stable Warm train 최소 작품 수 기준 충족, 평가셋 최소 rows 충족, 1작가 1작품 비율 기록 | 검증 완료 | split 생성 검증 | 상태 `pass`, test_warm `608`건, test_cold `3,342`건, Cold 이름 겹침 0 | T6-E001 | T6-E002 baseline 진행 |
