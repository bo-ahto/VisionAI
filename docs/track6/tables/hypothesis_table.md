...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| T6-H1 | T6-G1 | strict cold와 Stable Warm 기준을 적용한 Track6 split이 최종 보고 기준으로 더 적합할 것이다 | Track3/4/5 방법을 반영해 클렌징 후보를 확정하고 validation/test 우선 split으로 한글명, 동명이인, Stable Warm train 작품 수, Cold 이름 중복, 작가당 평가 작품 수, 컬럼 품질, feature/label 분리를 검증 | Track6 split v2 | split metadata, column quality metadata, feature/label manifest | Track5 split | Cold 이름 중복 0, Stable Warm train 최소 작품 수 기준 충족, 평가셋 최소 rows 충족, 1작가 1작품 비율 기록, 컬럼 품질 fail 0, feature 누수 컬럼 0 | 검증 완료 | split + 컬럼 품질 + feature/label 검증 | split `pass`, 컬럼 품질 fail 0 / review 17, feature/label `pass`, val/test 규모 근접 | T6-E001, T6-E001B, T6-E001C | T6-E002 baseline 진행 |
...
| T6-H5 | T6-G5 | 크기/재료/지지체 조합 피처는 일부 구간 성능을 개선할 수 있다 | 기본 피처에 조합 피처를 하나씩 추가하고 Warm/Cold validation 성능 비교 | Track6 split | 조합 피처 | 기본 피처셋 | median 또는 p95 개선 | 예정 | 미실행 | split 생성 후 진행 | T6-E005 | - |
