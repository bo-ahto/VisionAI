...
- 핵심 변경 (2026-06-13, WMIN8): 이전 운영 후보(방향 분류 + 미세 보정)와 달리, 유사작품 비교군 매칭의 최소 표본을 5에서 1로 낮춰 정밀 비교군을 더 많이 활용하고, 위험한 작품에만 보수적 대안 가중으로 조건부 교체하는 구조
- 정밀 비교군 매칭률: 최소 표본 5 기준 40.7% → 최소 표본 1 기준 81.9% (fixed test 작품 기준)
...
- 이전 운영 후보와의 차이: 1~3순위(작가 비교군)의 최소 표본이 5에서 1로 완화됨. 1건짜리 비교군도 인정하되, 학습 데이터에서는 자기 fold를 제외해 자기 가격이 자기 피처에 들어가지 않도록 함
- 자기가격 누설 점검: 자기 fold 제외 후 비교군이 0건이 되는 행은 다음 우선순위로 fallback (WMIN2에서 행별 확인 완료)
...
| 가중 혼합 | 비교군 기준가격과 방어형블렌드를 가중 혼합 | 기본 w700 / 대안 w850 |
...
- 보정 강도와 상한은 검증에서 고정된 안정 설정 (`alpha=0.01, cap=0.05, strength=0.50`)을 사용
...
- Warm fixed test 607건 기준 최종 성능 (WMIN8 채택 후보):
...
- 반복 안정성: artist 그룹 holdout에서 이전 운영 후보 대비 MAPE 승률 1.000, p95 승률 0.988
- 0604 신규 라벨 stress: 이전 운영 후보 대비 명확한 악화 없음 (MAPE delta -0.058, p95 delta -0.084) — 정밀 비교군 확대가 신규 라벨에서도 안전함을 확인
- 서비스 통합 검증: 동결 번들과 실험 산출물의 예측 일치 (재현 max 로그 차이 5.3e-15)
...
### 9.1 실서비스 운영 로그 모니터링 연동

- 모니터링 대상: 실서비스 운영 DB(`price_prediction_v0_1.sqlite`)의 라우팅 로그를 모니터링 하니스에 직접 연동해 5개 항목(R1~R5)을 상시 점검
- 연동 결과 (운영 라우팅 로그 637건 기준, 데이터 가공 없음):

| 점검 항목 | 결과 | 해석 |
|---|---|---|
| R1 라우팅 규칙 위반 | 0건 | 운영 라우팅이 매칭 임계 0.80과 이력 조건을 정확히 준수 |
...
- 하니스와 실서비스 연동 자체는 검증 완료 — 라우팅 규칙 준수(R1 0건)와 트래픽 분포(R2)는 실데이터로 정상 확인됨
...
- 동명이인율 경보 기준: 사전 밖 동명이인율이 16.7%(라우팅 허용 한계)를 넘으면 매칭 임계 0.80 재검토 경보 발생
...
- 라우팅 운영: 매칭 임계 0.80(경계 비교 tolerance 필수), 모니터링 하니스(`monitor_warm_lite_routing.py`)로 규칙 위반·트래픽 분포·성능 경보 상시 점검 — 실서비스 운영 로그(637건) 연동 검증 완료(R1 위반 0건), 상세 9.1절
