...
|---|---|---|---|---|---|
| 2026-06-07 | PP-SVCSHRINK3 | svc_numeric 전체 재현 + shrunk median 운영 교체 결정 | 실행 완료 | svc1 svc_numeric 전체 피처(warm base 13 + 비교군 stats) 재현, median만 EB-shrunk로 교체. **조건부 채택**: held-out test_warm+0604 전 지표 shrunk 지배(test_warm 0.1464/0.281/0.933 vs raw 0.1528/0.296/0.969; 0604 0.3427/0.434/0.999/std1.379 vs raw 0.3574/0.451/1.000/std1.640). 단 validation MdAPE 소폭 악화 + 반복 holdout 중립(MAPE 0.53, artist scheme 0.625)=center-vs-tail 트레이드오프. 운영 반영 전 seed_mean 전체 재현+고정 split 최종 확인 권고 | [요약](pp_svcshrink3_warm_svc_numeric_shrunk_operational_decision_summary.md), [실험 보고서](../../../experiments/track6/PP-SVCSHRINK3_warm_svc_numeric_shrunk_operational_decision/reports/PP-SVCSHRINK3_warm_svc_numeric_shrunk_operational_decision.md), [산출물](../../../experiments/track6/PP-SVCSHRINK3_warm_svc_numeric_shrunk_operational_decision/outputs/region_metrics.csv) |
| 2026-06-07 | PP-SVCSHRINK2 | Warm Huber + shrunk 비교군 median 재학습 | 실행 완료 | SVCSHRINK1 shrunk median을 Warm Huber(svc1 재현, 공통 9피처 base) 입력으로 투입. **채택** — shrunk가 held-out warm 평가 전 지표에서 raw median 지배: test_warm MAPE 0.429→0.371, 0604 MAPE 0.784→0.565/p95 1.179→1.000/std 2.108→1.447. (artist GroupKFold는 작가 제거=cold-like라 작가기반 비교군 피처엔 부적합 검증, 중립은 예상) | [요약](pp_svcshrink2_warm_huber_shrunk_comparable_refit_summary.md), [실험 보고서](../../../experiments/track6/PP-SVCSHRINK2_warm_huber_shrunk_comparable_refit/reports/PP-SVCSHRINK2_warm_huber_shrunk_comparable_refit.md), [산출물](../../../experiments/track6/PP-SVCSHRINK2_warm_huber_shrunk_comparable_refit/outputs/region_model_metrics.csv) |
| 2026-06-07 | PP-SVCSHRINK1 | Warm 비교군 prior shrinkage 갱신 | 실행 완료 | 거래 시점 데이터 부재로 literal 최근거래 갱신 불가 → PP-SVC8 staleness(분산)를 EB 계층 shrinkage로 완화. raw 비교군 prior를 작은/거친 그룹은 상위로 수축(k=5, validation 선택). **0604에서 raw 대비 residual std 1.097→0.949, MAPE 0.955→0.812, p95 4.017→3.358, MdAPE 0.394→0.384 개선(test 비악화)**. 커버리지 재확인(0604 L3최정밀 11.6% vs test 41%). 단 raw prior 컴포넌트 수준 → svc Huber 재학습+반복검증 후속 필요 | [요약](pp_svcshrink1_warm_comparable_prior_shrinkage_summary.md), [실험 보고서](../../../experiments/track6/PP-SVCSHRINK1_warm_comparable_prior_shrinkage/reports/PP-SVCSHRINK1_warm_comparable_prior_shrinkage.md), [산출물](../../../experiments/track6/PP-SVCSHRINK1_warm_comparable_prior_shrinkage/outputs/region_prior_metrics.csv) |
| 2026-06-07 | PP-COLD-ARTIFACT3 | Cold guard+search 2단 방어 artifact 고정 | 실행 완료 | PP-COLD-DEFENSE1 검증 guard+search를 `models/track6/cold_prediction_v0.3/`로 고정. 대표 PP-Y18 + 방어(guard + 작가단위 검색 delta lookup 372작가, 미커버→guard fallback) + 검수 플래그(qwidth 높음 OR 미커버, test 45%). 후처리기가 DEFENSE1 guard_search 지표(0.4098/0.849/2.347) 정확 재현(diff 3.6e-15). 정직한 범위: 후처리층만 실행, PP-Y18은 상류 search 의존, 검색 delta는 작가 frozen snapshot | [릴리스](../../../models/track6/cold_prediction_v0.3/reports/cold_artifact_release_v0_3.md), [정책](../../../models/track6/cold_prediction_v0.3/config/cold_model_policy_v0_3.json), [설계서](PP-COLD-ARTIFACT3_cold_two_layer_defense_freeze_plan.md) |
| 2026-06-07 | PP-COLD-DEFENSE1 | Cold guard + 검색 방어층 결합 | 실행 완료 | PP-QR4 guard + PP-H28 검색 보정을 PP-Y18 base에 결합. **가산적(중복 아님)**: redundancy gap +0.0006≈0, 두 방어 거의 직교(guard=qwidth/gap tail, search=작가맥락 잔차). guard_search_gm test 0.4098/0.849/2.347 = **Cold 최고**(base 0.4247/0.991/3.305 대비 MdAPE -3.5%/MAPE -14%/p95 -29%), guard 단독 대비 3지표 추가 개선. 단 검색층은 분산 추가(validation fold 개선확률 guard 1.00 vs 결합 0.72) + 커버리지 주의 | [요약](pp_cold_defense1_guard_search_layer_combination_summary.md), [실험 보고서](../../../experiments/track6/PP-COLD-DEFENSE1_cold_guard_search_layer_combination/reports/PP-COLD-DEFENSE1_cold_guard_search_layer_combination.md), [산출물](../../../experiments/track6/PP-COLD-DEFENSE1_cold_guard_search_layer_combination/outputs/additivity_decomposition.csv) |
