# Warm Huber HCOEF38 이후 지속 실험 /goal 프롬프트
...
- 최신 확인 결과: `PP-HCOEF38`
  - 목적: HCOEF37에서 부족했던 all3 안정성을 높이기 위해 적용 구간을 더 엄격하게 제한.
  - 최상위 후보: `hcoef38_route_best_mdape_all_cap0p01_s0p35__n_ge10_spread_q66_area80`.
  - fixed test: MdAPE `0.138803`, MAPE `0.272806`, p95_APE `0.806366`.
  - 0604 stress: MdAPE `0.273062`, MAPE `0.374425`, p95_APE `0.983456`.
  - 반복 검증: min stable any2/all3 `0.7167 / 0.2167`.
  - 판단: p95는 방어했지만 HCOEF37보다 반복 안정성이 약해 운영 후보로 채택하지 않음.

## 2. HCOEF38까지의 판단

- HCOEF36/37:
  - fixed test에서는 `hcoef_stable`보다 MdAPE/MAPE/p95가 모두 소폭 개선됨.
  - HCOEF37 최상위 후보의 min stable any2/all3는 `0.9333 / 0.4333`.
  - 두 지표 이상 개선 신호는 있으나 세 지표 동시 개선 안정성은 부족함.

- HCOEF38:
  - 적용 구간을 더 좁히면 p95는 방어됨.
  - 하지만 stable 대비 repeated any2/all3가 `0.7167 / 0.2167` 수준으로 낮아짐.
  - 라우팅을 더 조이는 방향만으로는 운영 후보 승격 가능성이 낮음.

- 다음 방향:
  - `PP-HCOEF39`: 기준가 생성 방식 재탐색.
  - `PP-HCOEF40`: Huber 저차원 계수 구조 재설계.
  - `PP-HCOEF41`: 목적별 후보 분리.
  - `PP-HCOEF42`: 점 예측 유지 + 가격 범위/신뢰도 정책.
...
- HCOEF38 결과상 stricter routing을 더 반복하는 것은 우선순위를 낮춘다.
...
목표는 fixed test 한 번의 점수를 좋게 만드는 것이 아니라, 반복 검증과 운영 안정성까지 통과하는 최고 성능 후보를 찾는 것이다. 실험은 기존 HCOEF 실험 구조를 유지하고, 마지막 완료 번호인 PP-HCOEF38 이후부터 이어서 관리해줘.
...
- docs/track6/experiments/warm_huber_hcoef38_continuation_goal_prompt.md
...
3. 최신 실험 PP-HCOEF38의 결론은 다음과 같다.
   - stricter low-risk routing은 p95를 방어했지만 반복 안정성이 HCOEF37보다 약해졌다.
   - 최상위 후보 fixed test는 MdAPE 0.138803, MAPE 0.272806, p95_APE 0.806366이다.
   - repeated min stable any2/all3는 0.7167 / 0.2167이다.
   - 따라서 라우팅을 더 조이는 실험은 우선순위를 낮춘다.
...
1. PP-HCOEF39: 기준가 생성 방식 재탐색
   ...
   - 핵심은 더 많은 세부 기준가를 만드는 것이 아니라 표본 부족 시 얼마나 보수적으로 상위 기준으로 fallback할지 검증하는 것이다.

2. PP-HCOEF40: Huber 계수 세분화
...
3. PP-HCOEF41: 목적별 후보 분리
   - MdAPE 최적 후보, MAPE 최적 후보, p95 방어 후보를 분리해서 만든다.
   - 운영 후보는 세 지표가 모두 안정적인 후보만 채택한다.

4. PP-HCOEF42: 점 예측 유지 + 가격 범위/신뢰도 정책
   - 점 예측 이동이 all3 안정성 기준을 통과하지 못하면 `hcoef_stable`을 유지한다.
   - quantile width, 기준가 spread, fallback level, 표본 수로 가격 범위와 신뢰도 tier를 고도화한다.

실험 관리 규칙은 아래처럼 지켜줘.

- `experiments/track6/` 아래에 실험별 독립 폴더를 만든다.
- 새 실험은 `PP-HCOEF##_짧은_설명` 형식으로 관리한다.
- 기존 실험 번호와 충돌하지 않게 PP-HCOEF39부터 시작한다.
