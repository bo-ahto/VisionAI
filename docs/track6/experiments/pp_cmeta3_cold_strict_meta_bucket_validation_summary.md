...
- 목적: `artist_key`를 참고하지 않고, 작가 메타정보 + 작품정보 + 검색/전시 정보만으로 Cold 성능이 어디까지 나오는지 확인한다.
...
- 허용 정보는 작품정보, 비가격성 작가 메타, 검색 문맥, 전시/갤러리 피처, 그리고 이 정보에서 만든 bucket이다.

## 2. 실험 순서

1. `PP-CMETA1` strict 최상위 후보를 재현했다.
2. 작가 메타 bucket을 추가했다.
3. 검색/전시 문맥 bucket을 추가했다.
4. 작품 bucket x 메타/search bucket 조합 피처를 추가했다.
...
|---|---|---:|---:|---:|---:|---|
| `search_external_bucket` | base q50 | 0.440549 | 1.037633 | 3.394335 | 0.857290 | test 기준 최상위. 검색/전시 bucket으로 중앙·평균 오차 소폭 개선 |
...
검색/전시 bucket은 가능성이 있다. `search_external_bucket`은 strict 조건에서 `artist_key` 없이 MdAPE와 MAPE를 기준보다 소폭 개선했다. 다만 p95가 악화되어 바로 최종 Cold 후보로 승격하기에는 tail 검증이 더 필요하다.
...
4. live search 피처가 동결 cache와 같은 schema/분포를 유지하는지 parity 검증한다.
5. 지금 단계에서는 official Cold 기본값을 변경하지 않고, strict Cold 차기 후보로 관리한다.

## 7. 결론

`artist_key` 없이 작가 메타정보와 작품정보만으로 성능을 확인하는 실험은 진행됐다. 결과적으로 작가 메타 bucket 단독은 충분하지 않았고, 검색/전시 bucket을 함께 넣은 후보가 strict 조건에서 가장 설득력 있는 개선을 보였다.
