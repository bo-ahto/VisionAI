...
## Warm-lite 절대 성능 (test 607행, 절단 seed 3개 평균 ± 표준편차)

| 이력 k | Warm-lite MdAPE | MAPE | p95 | (참조) Cold MdAPE/p95 |
|---|---|---|---|---|
| 1건 | 0.2179 ±0.013 | 0.3955 | 1.347 ±0.236 | 0.4947 / 2.70 |
| 2건 | 0.1739 ±0.003 | 0.3463 | 1.040 ±0.198 | 0.4911 / 2.81 |
| 3건 | 0.1597 ±0.012 | 0.3231 | 0.964 ±0.047 | 0.4884 / 2.76 |
| 4건 | 0.1457 ±0.001 | 0.3078 | 0.939 ±0.076 | 0.4704 / 2.52 |

- 위치: 기존 Warm(이력 5+, 운영) test `0.141/0.270/0.807` — **k=4면 기존 Warm MdAPE에 사실상 도달(0.146)**, k=2~3도 0.16~0.17로 Warm권. k=1만 0.218로 한 단계 아래이며 p95 변동(±0.24)이 커서 차등 정책(보수 cap/넓은 범위 표시) 필요.
- 같은 행 Cold 대비 MdAPE 2.3~3.2배 개선.
- 한계: 5+ 보유 작가의 절단 시뮬레이션이라 진짜 1~4건 작가(신진·무명)에서는 낙관 추정일 수 있음. 선형 proxy 기준(운영형 보정 체계 추가 시 상향 여지). 1~4건 트래픽 비중은 운영 데이터로 확인 필요.
