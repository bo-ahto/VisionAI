...
- `source`, `source_file`, `track4_source`는 학습 피처로 사용하지 않음
- 출처 정보는 원본 추적, 데이터 품질 감사, 분포 확인에만 사용함
- 출처별 가격대 차이가 있더라도 모델에 출처를 넣어 보정하지 않음
...
|---|---|---|---|
| `source` | 관리용 출처 | `track4_source` | 출처별 row 수 확인, 학습 피처 사용 금지 |
...
| T4-C6 | 출처 편향 점검 | 출처별 가격대 차이를 확인해야 데이터 오염과 분포 편향을 관리할 수 있다 | source별 가격/크기/재료 분포 비교 | source를 피처로 쓰지 않고도 품질관리 규칙을 설명 가능 |
...
- 6순위: 갤러리/출처 메타
- 이유: 운영 입력 피처로 쓰지 않지만 데이터 품질 판단과 원본 추적에는 필요함
- 먼저 확인할 항목
- gallery_name 결측
...
- 출처별 비교는 데이터 품질 점검용이며 모델 입력값 선정 근거로 사용하지 않음
