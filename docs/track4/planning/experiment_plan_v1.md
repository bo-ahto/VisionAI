...
- Track 3 가설은 그대로 복사하지 않음
- Track 4는 1차 시장 데이터로 다시 만든 데이터셋이므로 아래 차이를 반영함
- Warm 검증셋이 작아 반복 검증이 더 중요함
- 출처/갤러리 정보는 운영 입력으로 쓸 수 없어 피처에서 제외함
- 지지체 unknown 비율이 높아 결측/unknown 처리 가설이 중요함
- 3D/depth 극단 후보가 남아 2D/3D 조건부 실험이 필요함

| 우선순위 | 가설 ID | 세부 목표 | 가설 요약 | 상태 |
|---:|---|---|---|---|
| 1 | T4-H1 | 기본 예측 가능성 확인 | 새 데이터셋에서도 작품 구조 정보만으로 Warm / Cold 기본 예측이 가능한지 확인 | 예정 |
| 2 | T4-H2, T4-H20 | Warm 성능 개선 | Warm에서 작가 정보와 작가 이력 피처가 실제로 성능을 개선하는지 확인 | 예정 |
| 3 | T4-H4 | Cold 성능 개선 | Cold에서 robust 선형 계열과 트리 모델 중 더 안정적인 후보를 확인 | 예정 |
| 4 | T4-H6, T4-H14 | 운영 가능 피처 선정 | support unknown을 어떻게 처리해야 하는지 확인 | 예정 |
| 5 | T4-H8, T4-H16, T4-H27 | 약점 구간 보완 | 3D/depth 작품은 별도 피처 또는 조건부 fallback이 필요한지 확인 | 예정 |
| 6 | T4-H10, T4-H19 | Warm / Cold 라우팅 | 작가 존재 여부 외에 작가 작품 수 기준이 필요한지 확인 | 예정 |
| 7 | T4-H11, T4-H18, T4-H29 | 가격 범위/신뢰도 대응 | 단일 가격 대신 가격 범위와 신뢰도 등급을 어떻게 만들지 확인 | 예정 |
| 8 | T4-H21, T4-H22 | 모델 안정성 확인 | 공유 모델과 분리 모델, 반복 검증 안정성을 확인 | 예정 |
| 9 | T4-H25, T4-H30 | 최종 운영 정책 결정 | 금지 피처 검사와 재현 가능한 운영 패키지 조건을 확인 | 예정 |
