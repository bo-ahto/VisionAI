...
| T4-H23 | T4-G4, T4-G6 | 출처별 분포 차이는 피처로 쓰지 않더라도 감사 slice로 확인해야 한다 | source별 성능/결측/가격 분포를 평가만 하고 모델 입력에서는 제외 | validation/test split | source는 분석용 slice만 사용 | source 미사용 모델 | 특정 출처에서만 좋은 모델을 걸러냄 | 부분 검증 | validation 1회 | Cold에서 source별 성능 차이가 커서 결과 해석 주의 필요, source 피처 제외 원칙 유지 | T4-E036 | test/source slice 재확인 |
...
| T4-H25 | T4-G8 | 모델 학습 전 금지 피처 manifest를 검사하면 source/gallery 누수를 줄일 수 있다 | 학습 스크립트에서 금지 컬럼 포함 여부를 자동 검사 | 모델 학습 입력 | forbidden feature manifest | 수동 점검 | 금지 피처 포함 시 학습 중단 | 검증 완료 | manifest 자동 검사 | 현재 Warm/Cold 후보 피처셋 통과, source/price/gallery negative control 차단 확인 | T4-E034 | 최종 운영 dry-run에서 필수 단계로 연결 |
...
| T4-H28 | T4-G4, T4-G5 | 재료와 크기를 조합한 피처는 단독 피처보다 일부 약점 구간을 더 잘 설명할 수 있다 | `medium_size_bucket`, `large_oil`, `small_print` 등 조합 피처 추가 후 ablation | `track4_train`, validation split | medium-size/support combo | 단독 피처 모델 | Warm/Cold 한쪽 개선 시 모델별 분리 적용 | 부분 검증 | validation 1회 | Cold에서 medium-size 조합 개선 신호 있음, Warm 기본 채택은 보류 | T4-E035 | Quantile 후보와 같은 기준으로 재검토 |
