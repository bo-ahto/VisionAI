#!/usr/bin/env python3
"""Evaluate low/small overprediction guard candidates on existing v0.1 splits.

The correction tables are learned on the warm validation split and evaluated on
the warm test split. The 0604 labels are intentionally not used for calibration.
"""
...
<head>
<meta charset="utf-8">
<title>OP-V01-CAL-02 저가/소형 과대 예측 방어</title>
<style>
...
def add_bins(df: pd.DataFrame, pred_edges: np.ndarray | None = None) -> tuple[pd.DataFrame, np.ndarray]:
    out = df.copy()
    if pred_edges is None:
        pred_edges = np.quantile(out.loc[out["split"] == "validation", "pred_log"], [0, 0.2, 0.4, 0.6, 0.8, 1])
...
def learn_table(
    ...
    if negative_only:
        table["correction_log"] = np.minimum(table["correction_log"], 0.0)
    table["correction_log"] = table["correction_log"].clip(-cap_abs, cap_abs)
    return table
...
def apply_candidate(frame: pd.DataFrame, val: pd.DataFrame, name: str) -> tuple[pd.Series, pd.DataFrame]:
    ...
    if name == "global_negative_cap20":
        corr = min(float(val["residual_log"].median()), 0.0)
        corr = float(np.clip(corr, -0.20, 0.20))
        table = pd.DataFrame({"segment": ["global"], "n": [len(val)], "correction_log": [corr]})
        return frame["pred_log"] + corr, table
    specs = {
        "pred_bin_negative_cap20": (["pred_bin"], 30, 0.20),
        "area_bin_negative_cap20": (["area_bin"], 30, 0.20),
        "pred_area_negative_min20_cap20": (["pred_bin", "area_bin"], 20, 0.20),
        "pred_area_sample_negative_min15_cap15": (["pred_bin", "area_bin", "sample_bin"], 15, 0.15),
    }
    keys, min_n, cap_abs = specs[name]
    table = learn_table(val, keys=keys, min_n=min_n, cap_abs=cap_abs, negative_only=True)
...
def build_segment_error_summary(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for keys in [["pred_bin"], ["area_bin"], ["sample_bin"], ["pred_bin", "area_bin"]]:
...
def build_report(metrics: pd.DataFrame, segment_summary: pd.DataFrame, best_name: str, best_errors: pd.DataFrame) -> str:
    ...
    verdict = "채택 보류"
    if delta_mape <= -0.005 and delta_mdape <= 0 and delta_p95 <= 0 and delta_over < 0:
        verdict = "점가격 보정 후보"
    elif delta_mape < 0 and delta_mdape <= 0 and delta_p95 <= 0 and delta_over == 0:
        verdict = "미세 개선 후보, over 3x가 줄지 않아 기본 보정 채택 보류"
    elif delta_over < 0 or delta_p95 < 0:
        verdict = "범위/신뢰도 보정 후보"

    report = f"""# OP-V01-CAL-02 저가/소형 작품 과대 예측 방어 결과
...
{md_table(format_metrics(segment_summary.head(40)))}
...
def main() -> None:
    ...
    pred = load_base_predictions()
    features = load_features()
    df = pred.merge(features, on=["split", "_track6_row_id"], how="left")
    df, _ = add_bins(df)
    ...
    candidates = [
        "baseline",
        "global_negative_cap20",
        "pred_bin_negative_cap20",
        "area_bin_negative_cap20",
        "pred_area_negative_min20_cap20",
        "pred_area_sample_negative_min15_cap15",
    ]
    metric_rows = []
    corrected_frames = []
    ...
    ranked = ranked.sort_values(["MAPE", "p95_APE", "MdAPE"])
    best_name = ranked.iloc[0]["candidate"]

    segment_summary = build_segment_error_summary(val).sort_values(
        ["over_3x_n", "MAPE"], ascending=[False, False]
    )
    ...
    best_pred_log_col = f"{best_name}_pred_log"
    best_errors = selected_errors(pred_wide, best_pred_log_col)
    ...
    segment_summary.to_csv(OUTPUT_DIR / "validation_segment_error_summary.csv", index=False)
    ...
    for name, table in table_outputs.items():
        table.to_csv(ARTIFACT_DIR / f"{name}_correction_table.csv", index=False)

    report = build_report(metrics, segment_summary, best_name, best_errors)
