...
| `freshness_label` (예: "운영 모델 기준, 최대 10일") | 서버 | 그대로 표시 |
...
| snapshot 서빙승인 | "이 snapshot을 서빙 대상으로 승인할까요?" | "승인하면 이 snapshot이 서빙 대상이 됩니다. 사용자 노출은 이후 모델 학습·승격 단계에서 반영됩니다." | 서빙승인 |
