...
- NANT 분류는 `normalized_artwork_staging` 생성 다음 단계에서 수행한다.
...
| `artwork_nant_classification` | normalized 작품 row에 active mapping version을 적용한 분류 결과 |
...
CSV import 또는 admin draft 생성
  ...
  -> 새 normalized row부터 active mapping version으로 분류
...
가격 환산(`price_conversion`)은 snapshot export 준비 과정의 일부로 유지한다. NANT 분류는 가격 환산과 독립적이지만, snapshot 포함 여부를 결정할 때 함께 사용한다.
...
3. 매칭된 row의 `nant_support`와 `nant_medium`을 저장한다.
4. `nant_category_key = nant_support + '|' + nant_medium`으로 만든다.
5. `nant_category_key`는 같은 version의 `nant_allowed_category`에 존재해야 한다.
6. 매칭 row의 `learning_excluded`를 작품 분류 결과에 복사한다.

매칭 실패 시 임의로 가장 가까운 재료에 넣지 않는다. `nant_mapping_status=unmapped`로 남기고, 학습 snapshot에서는 보류 또는 제외한다.
...
| NANT 매핑 실패 | `include_status=excluded` 또는 보류, `exclude_reason=nant_unmapped` | 보존 |
...
- `nant_mapping_version`, `nant_allowed_category`, `nant_material_mapping`, `artwork_nant_classification` DDL
...
- 원천 row는 삭제하지 않는다. 제외 여부는 snapshot과 분류 결과에 남긴다.
