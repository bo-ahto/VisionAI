"""V5 cycle evaluation framework — 코덱스 권고 표준 구현.

핵심 (V4 cycle 학습 + 코덱스 권고 반영):
- LAO split (artist-level GroupShuffleSplit, 코덱스 hard gate: artist_slug overlap=0)
- 48-cell segment grid: exposure (0/1-3/4-10/10+) × source × price tercile × career_stage avail
- Modality leakage prevention helpers (scaler/PCA fit on train-only)
- Bootstrap CI for ΔMdAPE
- Pass/fail gate evaluator (사전등록 gate 자동 평가)
